...
- Do not deploy production directly. Every completed development release handoff must include `ATLAS_DEPLOYMENT_STAGE=development SKIP_GIT_PUSH=1 VERSION_NAME=vX.Y.Z bash scripts/deploy_remote_source.sh` with the actual continuous platform version, plus the commit SHA and post-deploy verification/experiment sequence. Development skips the PostgreSQL backup; formal releases must use `ATLAS_DEPLOYMENT_STAGE=production` and complete the backup before switching files.
