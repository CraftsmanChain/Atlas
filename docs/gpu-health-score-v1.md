...
> 实现进度（2026-07-23）：`gpu-health-v1.1.0` 已实现 DCGM 主源、gpu_exporter 补充/降级备用、双源一致性校验、逐特征来源快照、六维确定性规则、A～D 数据置信度、规则命中明细和只读 API。最终风险等级同时受分数区间与规则严重度约束，以两者中更严重者为准，避免硬故障证据被平均分稀释。现场指标覆盖与实际阈值依据见 [DCGM 指标能力盘点](dcgm-metric-capability-2026-07-20.md) 和 [GPU 双源健康评分契约 v1.1](gpu-dual-source-health-v1.1.md)。
