...
| `feature-catalog v1.1.0` / `gpu-health v1.1.2` | 25 个规范特征、41 个 DCGM/gpu_exporter 源查询、单位规范化、DCGM 优先回退、双源取值审计与补充硬件规则 | gpu_exporter 成为必需补充源和降级备用，但等价指标不重复计分；数值差异不定性为异常 | 逐特征来源可审计，回退降低置信度，双源差异不生成问题或进入风险 |
