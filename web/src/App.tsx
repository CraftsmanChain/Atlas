...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, accuracyResponse, outcomeReportResponse, governanceResponse, labelManifestResponse, evidenceBundleResponse, dataDriftResponse, calibrationDriftResponse, featureDriftResponse, challengerResponse, validationReadinessResponse, outcomesResponse, resultsResponse, parityResponse, replayResponse, coverageResponse, shadowResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/accuracy'), fetch('/api/v1/prediction/outcome-report'), fetch('/api/v1/prediction/model-governance'), fetch('/api/v1/prediction/label-manifest'), fetch('/api/v1/prediction/evidence-bundle'), fetch('/api/v1/prediction/data-drift-report'), fetch('/api/v1/prediction/calibration-drift-report'), fetch('/api/v1/prediction/feature-drift-report'), fetch('/api/v1/prediction/hearank-challenger'), fetch('/api/v1/prediction/validation-readiness'), fetch('/api/v1/prediction/outcomes'), fetch('/api/v1/prediction/results'), fetch('/api/v1/prediction/feature-parity'), fetch('/api/v1/prediction/history/feature-replays'), fetch('/api/v1/prediction/history/live-coverage'), fetch('/api/v1/prediction/history/shadow-scoring'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        ...
        const [overviewPayload, readinessPayload, labelsPayload, accuracyPayload, outcomeReportPayload, governancePayload, labelManifestPayload, evidenceBundlePayload, dataDriftPayload, calibrationDriftPayload, featureDriftPayload, challengerPayload, validationReadinessPayload, outcomesPayload, resultsPayload, parityPayload, replayPayload, coveragePayload, shadowPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), accuracyResponse.json(), outcomeReportResponse.json(), governanceResponse.json(), labelManifestResponse.json(), evidenceBundleResponse.json(), dataDriftResponse.json(), calibrationDriftResponse.json(), featureDriftResponse.json(), challengerResponse.json(), validationReadinessResponse.json(), outcomesResponse.json(), resultsResponse.json(), parityResponse.json(), replayResponse.json(), coverageResponse.json(), shadowResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.17.0',
    status: tx('特征分布漂移计算', 'FEATURE DISTRIBUTION DRIFT METRICS'),
    desc: tx(
