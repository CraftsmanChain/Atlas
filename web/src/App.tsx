...
type TemporalProbabilityQuality = { tp: number; fp: number; fn: number; tn: number; evaluated: number; precision?: number; recall?: number; accuracy?: number; brier_score?: number; null_brier_score?: number; brier_skill_score?: number; roc_auc?: number; pr_auc_average_precision?: number; expected_calibration_error?: number; scored_rows: number; invalid_probability_rows: number; invalid_label_rows: number; calibration_bins: number; calibration_bins_used: number };
    ...
    return <div className="grid prediction-page">
      ...
      {dualTrackValidation ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('概率质量', 'Probability quality')}</b><strong>ROC {pct(dualTrackValidation.probability_track.quality.roc_auc)} · AP {pct(dualTrackValidation.probability_track.quality.pr_auc_average_precision)}</strong><small>ECE {pct(dualTrackValidation.probability_track.quality.expected_calibration_error)} · BSS {pct(dualTrackValidation.probability_track.quality.brier_skill_score)} · {dualTrackValidation.probability_track.quality.scored_rows} SCORED</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.24.0',
    status: tx('概率质量指标', 'PROBABILITY QUALITY METRICS'),
    desc: tx(
