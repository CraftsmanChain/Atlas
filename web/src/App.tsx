    ...
    <main>
      ...
      <div className="content">
        ...
        <AnimatePresence mode="wait"><motion.div key={page} className={page === 'overview' ? 'overview-page' : 'secondary-page'} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .12 }}>
          ...
          {page === 'models' && <PageErrorBoundary key={subPage.models} title={tx('故障预警页面数据异常', 'Failure early-warning page data error')}><Models tx={tx} view={subPage.models} lang={lang} /></PageErrorBoundary>}
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  if (isPredictionView) {
    ...
    const scrollToPredictionSection = (id: string) => document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    const warningSections = [
      { id: 'prediction-feedback-package', label: tx('故障前后数据反馈包', 'Pre/Post-fault feedback pack'), note: tx('人工反馈、证据、结果闭环', 'Feedback, evidence and outcomes') },
      { id: 'prediction-human-feedback', label: tx('硬件故障人工反馈', 'Human fault feedback'), note: tx(`${humanFeedbackManifest?.human_confirmed_labels || 0} 条确认`, `${humanFeedbackManifest?.human_confirmed_labels || 0} confirmed`) },
      { id: 'prediction-readiness', label: tx('验证就绪门', 'Validation readiness'), note: validationReadiness?.status?.toUpperCase() || 'LOADING' },
      { id: 'prediction-outcomes', label: tx('准确率与改判', 'Accuracy & overrides'), note: tx(`${predictionAccuracy?.human_overrides || 0} 次人工改判`, `${predictionAccuracy?.human_overrides || 0} human overrides`) },
      { id: 'prediction-shadow-runtime', label: tx('只读影子运行', 'Read-only shadow runtime'), note: latestShadowScoring?.status?.toUpperCase() || tx('未运行', 'Not run') },
      { id: 'prediction-training-history', label: tx('历史训练链路', 'Training history'), note: latestTrainingMatrixBuild?.status?.toUpperCase() || tx('构建链路', 'Build pipeline') },
    ];
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="EARLY WARNING WORKBENCH" title={tx('故障预警子功能入口', 'Early-warning subfunctions')} action={<Badge value="READ-ONLY SHADOW" kind="info" />} />
        <p className="node-access-note">{tx('该页已按工作流拆成可直接定位的子功能：反馈包、人工反馈、验证就绪、准确率改判、影子运行和历史训练链路。所有入口仍是只读验证与证据治理，不会触发维修、隔离、调度或线上告警。', 'This page is split into direct workflow entry points: feedback pack, human feedback, validation readiness, accuracy overrides, shadow runtime, and training history. Every entry remains read-only validation and evidence governance; it triggers no repair, isolation, scheduling, or operational alert.')}</p>
        <div className="issue-categories quality-ledger">{warningSections.map(item => <button key={item.id} type="button" onClick={() => scrollToPredictionSection(item.id)}><span className="issue-category-icon"><BrainCircuit size={20} /></span><b>{item.label}</b><strong>{item.id === 'prediction-feedback-package' ? tx('重点', 'Priority') : 'OPEN'}</strong><small>{item.note}</small></button>)}</div>
      </Card>
      <Card id="prediction-feedback-package" className="span-12">
        <CardHead code="FAULT FEEDBACK PACK" title={tx('故障前后数据反馈包', 'Pre/Post-fault Data Feedback Pack')} action={<div className="table-actions"><Badge value={humanFeedbackManifest?.status?.toUpperCase() || 'WAITING'} kind={humanFeedbackManifest?.status === 'feedback_ready' ? 'healthy' : humanFeedbackManifest?.status === 'blocked' ? 'warning' : 'info'} /><Badge value="NO AUTO ACTION" kind="info" /></div>} />
        <p className="node-access-note">{tx('重点入口：把故障发生前的预测截点、特征/标签证据、故障发生后的人工确认、维修或换卡结论、outcome 改判和验证 blockers 放在同一个反馈包视角。它用于持续训练和验证闭环，不用于自动处置。', 'Priority entry: brings pre-fault prediction cutoffs, feature/label evidence, post-fault human confirmation, repair or replacement conclusions, outcome overrides, and validation blockers into one feedback-pack view. It feeds training and validation loops, not automatic remediation.')}</p>
        <div className="issue-categories quality-ledger">
          <div><b>{tx('人工确认 / 改判', 'Confirmed / overrides')}</b><strong>{humanFeedbackManifest ? `${humanFeedbackManifest.human_confirmed_labels} / ${humanFeedbackManifest.human_override_outcomes}` : '—'}</strong><small>{humanFeedbackManifest ? `${humanFeedbackManifest.confirmed_positive_labels} POS LABELS · ${humanFeedbackManifest.human_positive_outcomes} POS OUTCOMES` : tx('等待人工反馈 Manifest', 'Waiting for human feedback manifest')}</small></div>
          <div><b>{tx('前后窗口绑定', 'Pre/post window binding')}</b><strong>{humanFeedbackManifest ? `${humanFeedbackManifest.matched_prediction_windows}` : '—'}</strong><small>{humanFeedbackManifest ? `${humanFeedbackManifest.confirmed_labels_without_prediction_window_match} WITHOUT MATCH · ${humanFeedbackManifest.point_in_time_violations} PIT VIOLATIONS` : tx('等待预测窗口匹配', 'Waiting for prediction-window matching')}</small></div>
          <div><b>{tx('验证就绪', 'Validation readiness')}</b><strong>{validationReadiness?.status?.toUpperCase() || '—'}</strong><small>{validationReadiness?.readiness_sha256?.slice(0, 16) || tx('等待 readiness SHA', 'Waiting for readiness SHA')}</small></div>
          <div><b>{tx('反馈指纹', 'Feedback SHA')}</b><strong>{humanFeedbackManifest?.manifest_sha256?.slice(0, 16) || '—'}</strong><small>{humanFeedbackManifest ? `${time(humanFeedbackManifest.generated_at, lang)} · ${humanFeedbackManifest.framework_version}` : tx('无反馈包', 'No feedback pack')}</small></div>
        </div>
        <div className="prediction-contract">
          <span><b>{tx('人工反馈 Manifest', 'Human feedback manifest')}</b><a className="link" href="/api/v1/prediction/human-feedback-manifest?download=1" download><Download size={13} />{tx('下载', 'Download')}</a><small>{humanFeedbackManifest?.version || 'prediction-human-feedback-manifest-v1'}</small></span>
          <span><b>{tx('训练样本证据包', 'Training evidence bundle')}</b><a className="link" href="/api/v1/prediction/evidence-bundle?download=1" download><Download size={13} />{tx('下载', 'Download')}</a><small>{evidenceBundle?.bundle_sha256?.slice(0, 16) || 'NO SHA'}</small></span>
          <span><b>{tx('Outcome 报告', 'Outcome report')}</b><a className="link" href="/api/v1/prediction/outcome-report?download=1" download><Download size={13} />{tx('下载', 'Download')}</a><small>{predictionOutcomeReport?.version || 'prediction-outcome-report-v1'}</small></span>
          <span><b>{tx('验证就绪报告', 'Validation readiness')}</b><a className="link" href="/api/v1/prediction/readiness?download=1" download><Download size={13} />{tx('下载', 'Download')}</a><small>{validationReadiness?.human_feedback_manifest_sha256?.slice(0, 16) || 'NO FEEDBACK SHA'}</small></span>
        </div>
      </Card>
      ...
      {humanFeedbackManifest ? <Card id="prediction-human-feedback" className="span-12">
        <CardHead code={humanFeedbackManifest.version} title={tx('人工故障反馈 Manifest', 'Human Fault Feedback Manifest')} action={<div className="table-actions"><Badge value={humanFeedbackManifest.status.toUpperCase()} kind={humanFeedbackManifest.status === 'feedback_ready' ? 'healthy' : humanFeedbackManifest.status === 'blocked' ? 'warning' : 'info'} /><Badge value="NO AUTO ACTION" kind="info" /><a className="link" href="/api/v1/prediction/human-feedback-manifest?download=1" download><Download size={13} />{tx('下载反馈清单', 'Download feedback')}</a></div>} />
        <p className="node-access-note">{tx('汇总人工确认标签和人工 outcome 覆写，检查身份、时间可用性和预测窗口匹配；该报告只用于验证与未来训练证据归档，不触发告警、维修、隔离或调度。', 'Summarizes human-confirmed labels and human outcome overrides, checking identity, availability time, and prediction-window matches. This report is validation and future-training evidence only; it triggers no alert, repair, isolation, or scheduling.')}</p>
        <div className="issue-categories quality-ledger">
          <div><b>{tx('人工确认 / 覆写', 'Confirmed / Overrides')}</b><strong>{humanFeedbackManifest.human_confirmed_labels} / {humanFeedbackManifest.human_override_outcomes}</strong><small>{humanFeedbackManifest.confirmed_positive_labels} POS LABELS · {humanFeedbackManifest.human_positive_outcomes} POS OUTCOMES</small></div>
          <div><b>{tx('匹配预测窗口', 'Matched windows')}</b><strong>{humanFeedbackManifest.matched_positive_prediction_windows} / {humanFeedbackManifest.matched_prediction_windows}</strong><small>{humanFeedbackManifest.confirmed_labels_without_prediction_window_match} CONFIRMED WITHOUT MATCH</small></div>
          <div><b>{tx('元数据缺口', 'Metadata gaps')}</b><strong>{humanFeedbackManifest.labels_missing_identity + humanFeedbackManifest.labels_missing_available_at + humanFeedbackManifest.outcome_overrides_missing_operator}</strong><small>ID {humanFeedbackManifest.labels_missing_identity} · TIME {humanFeedbackManifest.labels_missing_available_at} · OPERATOR {humanFeedbackManifest.outcome_overrides_missing_operator}</small></div>
          <div><b>{tx('时间安全', 'Point-in-time safety')}</b><strong>{humanFeedbackManifest.point_in_time_violations}</strong><small>{humanFeedbackManifest.blocking_reasons[0] || humanFeedbackManifest.recommended_next_run[0] || humanFeedbackManifest.manifest_sha256.slice(0, 16)}</small></div>
        </div>
        <div className="prediction-contract">
          <span><b>{tx('反馈策略', 'Feedback policy')}</b>{humanFeedbackManifest.feedback_policy[0] || '—'}<small>{humanFeedbackManifest.feedback_policy.slice(1, 3).join(' · ')}</small></span>
          <span><b>{tx('事件分布', 'Event distribution')}</b>{Object.entries(humanFeedbackManifest.by_event_type || {}).slice(0, 2).map(([key, value]) => `${key} ${value}`).join(' · ') || '—'}<small>{Object.entries(humanFeedbackManifest.by_outcome || {}).slice(0, 3).map(([key, value]) => `${key} ${value}`).join(' · ') || humanFeedbackManifest.outcome_rule_version}</small></span>
          <span><b>{tx('样本记录', 'Sample record')}</b>{humanFeedbackManifest.sample_records[0]?.entity_key || '—'}<small>{humanFeedbackManifest.sample_records[0] ? `${humanFeedbackManifest.sample_records[0].source} #${humanFeedbackManifest.sample_records[0].reference_id} · ${humanFeedbackManifest.sample_records[0].status}` : tx('暂无人工反馈', 'No human feedback yet')}</small></span>
          <span><b>{tx('反馈指纹', 'Feedback SHA')}</b>{humanFeedbackManifest.manifest_sha256.slice(0, 16) || '—'}<small>{time(humanFeedbackManifest.generated_at, lang)} · {humanFeedbackManifest.framework_version}</small></span>
        </div>
      </Card> : null}
