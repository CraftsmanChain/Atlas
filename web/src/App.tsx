...
type NodeAccessOverview = {
  ...
  no_arbitrary_shell: boolean; no_change_executed: boolean; encryption_ready: boolean; management_ready: boolean; secure_write_only: boolean; insecure_http_allowed: boolean; connectivity_check_enabled: boolean; known_hosts_ready: boolean;
  ...
  if (view === 'node-access') {
    ...
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.3.0'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('v0.3.0 只允许对资产库中的非退役节点执行 known-host SSH 握手与认证检查；认证成功后立即断开，不创建 session、不执行命令。', 'v0.3.0 permits only known-host SSH handshake and authentication checks for non-retired managed assets; it disconnects immediately after authentication without opening a session or executing commands.')}</p></div></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'node-access', name: tx('节点证据 Skill', 'Node Evidence Skill'), version: 'v0.3.0', status: tx('known-host 认证基线', 'KNOWN-HOST AUTH BASELINE'), desc: tx('对 Atlas 资产库中的非退役节点执行 known-host SSH 握手与凭据轮换认证，成功后立即断开；审计结果不含凭据或远端输出，失败进入 access 问题台账。当前不创建 session、不执行命令。', 'Perform known-host SSH handshake and credential-rotation authentication only for non-retired Atlas-managed assets, then disconnect immediately. Audit results contain no credential or remote output, and failures enter the access issue ledger. No session is opened and no command is executed.'), history: [tx('v0.3.0 · 受管资产限制、known-host 校验、仅认证 API、审计记录和 access 问题自动开闭', 'v0.3.0 · managed-asset restriction, known-host verification, authentication-only API, audit records, and automatic access issue lifecycle'), tx('v0.2.1 · 增加默认关闭的 HTTP 兼容开关、API 状态与页面风险提示', 'v0.2.1 · added a default-off HTTP compatibility switch, API status, and UI risk warning'), tx('v0.2.0 · 页面加密录入、密文落库、管理口令写保护、掩码列表与认证前解密', 'v0.2.0 · encrypted page entry, ciphertext persistence, management-token write protection, masked lists, and pre-authentication decryption'), tx('v0.1.0 · Skill 契约、只读/审批命令分级、凭据引用轮换状态机与状态页面', 'v0.1.0 · Skill contract, read-only/approval command classes, credential-reference rotation state machine, and status UI')] },
