...
type HealthSummary = { total: number; scored: number; unknown: number; fallback_gpus: number; consistency_candidate_gpus: number; inconsistent_gpus: number; average_score: number; by_level: Record<string, number>; by_confidence: Record<string, number>; latest_run: { id: number; rule_version: string; status: string; rule_hit_count: number; finished_at?: string } | null };
...
function Gpus({ tx, view, assets, models, loading, inventoryError, healthScores, healthSummary, lang }: { tx: Tx; view: string; assets: GPUAsset[]; models: { model: string; short: string; count: number }[]; loading: boolean; inventoryError: boolean; healthScores: GPUHealthScore[]; healthSummary: HealthSummary | null; lang: string }) {
  ...
  if (view === 'health') {
    const risky = healthScores.filter(x => x.score == null || x.level !== 'healthy' || x.data_confidence !== 'A' || (x.fallback_metric_count || 0) > 0 || (x.consistency_issue_count || 0) > 0).sort((a, b) => {
    ...
    return <div className="grid">
      <section className="health-kpi-panel health-kpi-core">
      ...
      <section className="health-kpi-panel health-kpi-trust">
        <header><div><code>DATA CONFIDENCE</code><h2>{tx('数据可信度', 'Data Confidence')}</h2></div><small>{tx('补充源回退与跨源一致性', 'Fallback and cross-source consistency')}</small></header>
        <div className="health-kpi-grid">
          {[[tx('降级取值', 'Fallback GPUs'), String(healthSummary?.fallback_gpus ?? '—'), 'GPU_EXPORTER FALLBACK', Database], [tx('持续双源偏差', 'Persistent mismatch'), String(healthSummary?.inconsistent_gpus ?? '—'), `${healthSummary?.consistency_candidate_gpus ?? 0} ${tx('本轮候选', 'CURRENT CANDIDATES')}`, AlertTriangle]].map(([label, value, note, Icon]) => { const I = Icon as typeof Cpu; return <Card className="metric" key={String(label)}><I size={18} /><span>{String(label)}</span><strong>{String(value)}</strong><small>{String(note)}</small></Card>; })}
        </div>
      </section>
      <Card className="span-12"><CardHead code="RISK QUEUE" title={tx('GPU 健康与数据风险', 'GPU Health & Data Risks')} action={<Badge value={`${risky.length} / ${healthScores.length}`} kind={risky.length ? 'warning' : 'healthy'} />} /><div className="table-wrap"><table className="health-table"><thead><tr><th>{tx('节点 / GPU', 'Node / GPU')}</th><th>{tx('型号', 'Model')}</th><th>{tx('分数', 'Score')}</th><th>{tx('级别', 'Level')}</th><th>{tx('置信度', 'Confidence')}</th><th>{tx('健康领域分数', 'Health Domain Scores')}</th><th>{tx('详情', 'Details')}</th><th>{tx('计算时间', 'Evaluated')}</th></tr></thead><tbody>{risky.map(score => <tr key={score.gpu_asset_id}><td><b>{score.node_ip} · GPU {score.gpu_index}</b><small><code>{score.gpu_uuid}</code></small></td><td>{score.model_name}</td><td className="score-cell"><strong className="score-value">{score.score ?? '—'}</strong></td><td><Badge value={score.level.toUpperCase()} kind={tone(score.level)} /></td><td><Badge value={score.data_confidence} kind={score.data_confidence === 'A' ? 'healthy' : score.data_confidence === 'B' ? 'info' : score.data_confidence === 'C' ? 'warning' : 'danger'} /></td><td><div className="domain-scores">{dims.map(domain => <span key={domain.code}><b>{domain.code} {domain.name}</b><strong>{score[domain.score]}</strong></span>)}</div></td><td><small>{(score.evidence || []).join(' · ')}</small><small>{tx('数据源', 'Sources')}: {(score.sources_available || []).join(' + ') || '—'} · fallback {score.fallback_metric_count || 0} · mismatch {score.consistency_issue_count || 0}</small>{(score.consistency_issues || []).length > 0 && <small className="state-warning">{score.consistency_issues?.join(' · ')}</small>}</td><td>{time(score.evaluated_at, lang)}</td></tr>)}</tbody></table>{!loading && healthScores.length === 0 && <Empty tx={tx} title={tx('等待首次健康评分', 'Waiting for first health evaluation')} />}</div></Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.4.0', status: tx('双源质量基线', 'DUAL-SOURCE BASELINE'), desc: tx('识别 Target 异常、指标缺失或陈旧、覆盖不足、身份冲突、双源偏差与采集链路问题，并分别标识接收、资产与健康数据的新鲜度，不依赖节点登录权限。', 'Detect target failures, missing or stale metrics, coverage gaps, identity conflicts, cross-source mismatches and collection-path issues, with separate freshness states for ingestion, inventory and health data, without requiring node access.'), history: [tx('v0.4.0 · DCGM 与 gpu_exporter 规范化一致性校验及问题闭环', 'v0.4.0 · normalized DCGM/gpu_exporter consistency checks and issue workflow'), tx('v0.3.0 · 统一数据时效 API、SLA 边界和 8077 源时间标识', 'v0.3.0 · unified freshness API, SLA boundaries and source-time indicator on 8077'), tx('v0.2.0 · 原因分类、节点抑制与同步审计', 'v0.2.0 · reason classification, node suppression and sync audit'), tx('v0.1.0 · 四类 Target 覆盖矩阵', 'v0.1.0 · four-target coverage matrix')] },
    { id: 'health', name: tx('硬件健康评分', 'Hardware Health Scoring'), version: 'v1.1.1', status: tx('双源 GPU 基线', 'DUAL-SOURCE GPU BASELINE'), desc: tx('以 DCGM 为主、gpu_exporter 补充与降级备用，结合可解释规则、六个健康领域、来源血缘和数据置信度输出 GPU 当前健康状态。', 'Score current GPU health with DCGM primary and gpu_exporter supplements/fallbacks, using explainable rules, six health domains, source lineage and data confidence.'), history: [tx('v1.1.1 · 明确已评分平均分口径，分组展示健康结果与数据可信度', 'v1.1.1 · clarified the scored-average scope and grouped health outcomes separately from data confidence'), tx('v1.1.0 · gpu_exporter 补充特征、DCGM 优先降级备用、双源一致性与置信度降级', 'v1.1.0 · gpu_exporter supplements, DCGM-first fallback, consistency checks and confidence degradation'), tx('v1.0.0 · GPU 六领域规则评分与 A–D 置信度', 'v1.0.0 · six-domain GPU rules and A–D confidence')] },
    ...
    { id: 'issue', name: tx('问题统计与处置', 'Issue Analytics & Resolution'), version: 'v0.2.0', status: tx('双源问题闭环', 'SOURCE QUALITY WORKFLOW'), desc: tx('统一统计平台发现的问题、分类、自动检测状态和人工处置状态，纳入 GPU 双源不一致问题，记录根因、方案、处理过程与结果，并输出可审计的 AI/Skill 训练数据。', 'Normalize discovered problems, including GPU source inconsistencies, categories, automated detection and human workflow states; capture root cause, solution, process and result; and export audited AI/Skill training data.'), history: [tx('v0.2.0 · 双源一致性问题自动生成与恢复关闭', 'v0.2.0 · automatic source-consistency issue creation and recovery'), tx('v0.1.0 · 五类检测源归一化、统计钻取、处置历史与训练数据导出', 'v0.1.0 · five-source normalization, analytics drill-down, resolution history and training export')] },
