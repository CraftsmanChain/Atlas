...
type HistoricalFaultCandidate = { id: number; candidate_key: string; source_key: string; gpu_uuid: string; node_ip: string; hostname: string; model_name: string; pci_bus_id: string; event_type: string; event_code: string; event_message: string; severity: string; quality_tier: string; operational_priority: string; hardware_certainty: string; training_disposition: string; recommended_action: string; recovery_aware: boolean; review_status: string; reviewed_at?: string; reviewed_by?: string; review_note?: string; source_metric: string; source_alert_name: string; signal_samples: number; onset_at: string; detection_window_end_at: string };
type HistoricalCandidateSummary = { total: number; pending_review: number; by_review_status: Record<string, number>; by_event_code: Record<string, number>; by_quality_tier: Record<string, number>; by_operational_priority: Record<string, number>; by_hardware_certainty: Record<string, number>; by_training_disposition: Record<string, number>; by_model: Record<string, number>; earliest_onset_at?: string; latest_onset_at?: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        <p className="node-access-note">{tx('按 7 天分块、1 分钟步长扫描低/高优先 XID、恢复型 120/154、GPU 掉卡和不可恢复显存重映射行。处置优先级、硬件确定性和训练用途独立记录，候选不会自动成为确认标签。', 'Scan low/high-priority XIDs, recovery-latched 120/154, GPU dropout and uncorrectable remapped rows in seven-day chunks at one-minute resolution. Operational priority, hardware certainty and training use are recorded independently; candidates are never automatically confirmed.')}</p>
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('确定性硬件', 'Deterministic')}</b><strong>{historicalCandidateSummary?.by_hardware_certainty?.deterministic_hardware || 0}</strong><small>DROPOUT / MEMORY</small></div>
        ...
        <div className="table-wrap"><table className="prediction-label-table"><thead><tr><th>{tx('发生时间', 'Onset')}</th><th>{tx('节点 / GPU', 'Node / GPU')}</th><th>{tx('事件', 'Event')}</th><th>{tx('处置 / 硬件', 'Operations / Hardware')}</th><th>{tx('训练用途 / 审核', 'Training / Review')}</th><th>{tx('历史来源', 'Historical Source')}</th></tr></thead><tbody>{historicalCandidates.map(candidate => <tr key={candidate.id}><td>{time(candidate.onset_at, lang)}</td><td><b>{candidate.node_ip || candidate.hostname || '—'}</b><small>{candidate.gpu_uuid || candidate.pci_bus_id || 'NODE-LEVEL'}</small></td><td><code>{candidate.event_type}</code><small>{candidate.event_message || candidate.event_code}</small></td><td><Badge value={(candidate.operational_priority || 'unknown').toUpperCase()} kind={candidate.operational_priority === 'critical' ? 'danger' : candidate.operational_priority === 'high' ? 'warning' : 'info'} /><small>{candidate.hardware_certainty || 'unclassified'}</small></td><td><Badge value={(candidate.training_disposition || candidate.quality_tier).toUpperCase()} kind={candidate.training_disposition === 'positive_after_identity_review' ? 'warning' : 'info'} /><small>{candidate.review_status.toUpperCase()}{candidate.reviewed_by ? ` · ${candidate.reviewed_by}` : ''}</small>{candidate.review_note ? <small title={candidate.review_note}>{candidate.review_note}</small> : null}<div className="table-actions"><button className="link" type="button" disabled={reviewingCandidateID === candidate.id} onClick={() => void reviewHistoricalCandidate(candidate, 'accepted_proxy')}>{tx('接受代理', 'Accept')}</button><button className="link" type="button" disabled={reviewingCandidateID === candidate.id} onClick={() => void reviewHistoricalCandidate(candidate, 'context_only')}>{tx('仅上下文', 'Context')}</button><button className="link" type="button" disabled={reviewingCandidateID === candidate.id} onClick={() => void reviewHistoricalCandidate(candidate, 'needs_evidence')}>{tx('补证据', 'Evidence')}</button><button className="link" type="button" disabled={reviewingCandidateID === candidate.id} onClick={() => void reviewHistoricalCandidate(candidate, 'excluded')}>{tx('排除', 'Exclude')}</button></div></td><td><small>{candidate.source_metric} · {candidate.source_alert_name || 'DIRECT METRIC'}</small><small>{candidate.signal_samples} ONSET SAMPLES{candidate.recovery_aware ? ' · RECOVERY-AWARE' : ''}</small></td></tr>)}</tbody></table>{historicalCandidates.length === 0 && <Empty tx={tx} title={tx('尚未执行历史候选重建', 'Historical candidate backfill has not run')} />}</div>
      </Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.6.0',
    status: tx('历史候选人工审核', 'HISTORICAL CANDIDATE REVIEW'),
    desc: tx(
      '在部署节点本地重建 GPU 故障候选，并通过可重审的人工结果控制接受代理、仅作上下文、补充证据或排除；审核保留操作人、时间和备注，候选不会自动升级为确认标签。',
      'Reconstruct GPU fault candidates locally and control proxy acceptance, context-only use, evidence requests or exclusion through repeatable human review. Reviews retain operator, time and notes, and never auto-upgrade candidates to confirmed labels.',
    ),
    history: [
