...
type PredictionFeatureDistributionArchiveMeta = { archive_version?: string; archive_sha256?: string; snapshot_count?: number; training_snapshot_count?: number; live_shadow_snapshot_count?: number; baseline_count?: number; feature_count?: number; latest_observed_at?: string; raw_samples_stored?: boolean; scoring_allowed?: boolean; alerts_emitted?: boolean; actions_executed?: boolean };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="FEATURE DISTRIBUTION SNAPSHOTS" title={tx('逐特征分布快照', 'Feature Distribution Snapshots')} action={<div className="table-actions"><Badge value="AGGREGATED ONLY" kind="info" /><Badge value={`${featureDistributionArchive?.snapshot_count ?? featureDistributionSnapshots.length} SNAPSHOTS`} kind={featureDistributionSnapshots.length ? 'healthy' : 'warning'} /><a className="link" href="/api/v1/prediction/history/feature-distributions?download=1" download><Download size={13} />{tx('下载快照归档', 'Download archive')}</a><button className="link" type="button" disabled={featureDistributionStarting || latestBaselineModelBuild?.status !== 'completed'} onClick={() => void materializeTrainingFeatureDistributions()}>{featureDistributionStarting ? tx('生成中…', 'Materializing…') : tx('生成训练分布', 'Materialize training')}</button></div>} />
        <p className="node-access-note">{tx('展示已持久化的训练分布与 live shadow 分布压缩快照；训练分布可从已完成 baseline 绑定的训练矩阵生成，live-shadow 分布由后续影子评分批次在内存聚合生成，只保存统计量和 histogram proportions，不保存原始遥测样本。快照归档提供稳定 SHA256 和机器可读下载，用于固定特征漂移验证证据。', 'Shows persisted compact training and live-shadow feature distribution snapshots. Training distributions can be materialized from the completed baseline-bound training matrix; live-shadow distributions are aggregated in memory by subsequent shadow scoring batches. Only aggregate statistics and histogram proportions are stored, never raw telemetry samples. The snapshot archive exposes a stable SHA256 and machine-readable download to freeze feature-drift validation evidence.')}</p>
        <div className="issue-categories quality-ledger">
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.5',
    status: tx('特征分布快照归档', 'FEATURE DISTRIBUTION ARCHIVE'),
    desc: tx(
