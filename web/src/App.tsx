  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        <p className="node-access-note">{tx('只对通过数据充分性门的预测窗分别训练零外部依赖 Logistic Regression；采样数仅用于质量门，XID、ECC、Row Remap 和 Reset 等故障已发生指标同样禁止入模。验证集选择优先满足 precision≥70%、recall≥50% 的阈值，独立测试集必须再次通过稳定性、校准及精确率/召回率门禁；不发布线上概率。', 'Train a dependency-free Logistic Regression separately only for horizons that pass data-sufficiency gates. Sample counts remain quality-only, while XID, ECC, row-remap, reset and other occurred-fault indicators are prohibited. Validation selects an operating point prioritizing precision ≥70% and recall ≥50%; the independent test must pass stability, calibration, precision, and recall gates again. No online probability is released.')}</p>
