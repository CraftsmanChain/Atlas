...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  const [feedbackForm, setFeedbackForm] = useState({ gpu_asset_id: '', fault_type: 'gpu_hardware_failure', fault_occurred_at: '', pre_window_hours: '72', post_window_hours: '24', operator: '', description: '', repair_action: 'pending', hardware_replaced: false, evidence_note: '', training_eligible: true });
  ...
  if (isPredictionView) {
    ...
    const replacementFeedback = feedbackForm.hardware_replaced || feedbackForm.repair_action === 'replace_gpu' || feedbackForm.repair_action === 'replace_node' || feedbackForm.repair_action === 'board_replacement';
    const warningTabs = [
    ...
    const submitHardwareFeedback = async () => {
      ...
      setPredictionError('');
      try {
        if (!selectedFeedbackAsset) throw new Error(tx('请选择发生硬件故障的机器和 GPU 卡', 'Select the failed node and GPU'));
        if (!feedbackForm.fault_occurred_at.trim()) throw new Error(tx('请填写故障发生时间', 'Fault time is required'));
        ...
        const response = await fetch('/api/v1/prediction/hardware-fault-feedback', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            gpu_asset_id: selectedFeedbackAsset.id,
            node_ip: selectedFeedbackAsset.node_ip,
            gpu_uuid: replacementFeedback ? '' : selectedFeedbackAsset.gpu_uuid,
            reported_gpu_uuid: selectedFeedbackAsset.gpu_uuid,
            gpu_index: selectedFeedbackAsset.gpu_index,
        ...
        setFeedbackForm(current => ({ ...current, fault_occurred_at: '', description: '', evidence_note: '' }));
    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'human-feedback' ? <Card id="prediction-human-feedback" className="span-12">
        ...
        <p className="node-access-note">{tx('这里录入“哪台机器、哪张卡、什么硬件故障、什么时候发生”。提交后只创建离线前后监控历史数据包采集请求，用于后续训练样本；不会自动维修、隔离、调度或发线上告警。', 'Record which node/GPU failed, the hardware fault type, and when it happened. Submitting creates an offline pre/post monitoring-history pack request for later training data only; it performs no repair, isolation, scheduling, or operational alerting.')}</p>
        <form className="settings-form fault-feedback-form" onSubmit={event => { event.preventDefault(); void submitHardwareFeedback(); }}>
          <label><span>{tx('搜索故障 GPU', 'Search failed GPU')}<small>IP / UUID / GPU INDEX / MODEL</small></span><input value={feedbackAssetQuery} onChange={event => setFeedbackAssetQuery(event.target.value)} placeholder={tx('输入节点 IP、GPU3、UUID 或型号过滤', 'Filter by node IP, GPU3, UUID or model')} /></label>
          <label><span>{tx('故障机器 / GPU', 'Failed node / GPU')}<small>REQUIRED · {filteredFeedbackAssets.length}/{activeAssets.length} GPU</small></span><select value={feedbackForm.gpu_asset_id} onChange={event => setFeedbackForm(current => ({ ...current, gpu_asset_id: event.target.value }))}><option value="">{tx('选择发生故障的 GPU', 'Select failed GPU')}</option>{feedbackAssetOptions.map(asset => <option key={asset.id} value={asset.id}>{asset.node_ip} · GPU {asset.gpu_index} · {asset.gpu_uuid || 'UUID UNKNOWN'} · {asset.model_name || asset.model || 'MODEL UNKNOWN'}</option>)}</select></label>
          <label><span>{tx('硬件故障类型', 'Hardware fault type')}<small>REQUIRED · LABEL</small></span><select value={feedbackForm.fault_type} onChange={event => setFeedbackForm(current => ({ ...current, fault_type: event.target.value }))}><option value="gpu_hardware_failure">{tx('GPU 硬件故障', 'GPU hardware failure')}</option><option value="gpu_memory_failure">{tx('GPU 显存故障', 'GPU memory failure')}</option><option value="pcie_link_failure">{tx('PCIe / 掉卡故障', 'PCIe / dropout failure')}</option><option value="thermal_power_failure">{tx('温度 / 供电相关故障', 'Thermal / power fault')}</option><option value="board_replacement">{tx('板卡更换确认', 'Board replacement confirmed')}</option></select></label>
          <label><span>{tx('故障发生时间', 'Fault occurred at')}<small>REQUIRED · LOCAL TIME</small></span><input type="datetime-local" value={feedbackForm.fault_occurred_at} onChange={event => setFeedbackForm(current => ({ ...current, fault_occurred_at: event.target.value }))} /></label>
          ...
          <label><span>{tx('维修/更换结论', 'Repair/replacement result')}<small>OPTIONAL · DISPOSITION</small></span><select value={feedbackForm.repair_action} onChange={event => setFeedbackForm(current => ({ ...current, repair_action: event.target.value, hardware_replaced: event.target.value === 'replace_gpu' || event.target.value === 'replace_node' || event.target.value === 'board_replacement' ? true : current.hardware_replaced }))}><option value="pending">{tx('待确认', 'Pending')}</option><option value="reseat">{tx('重新插拔 / 复位', 'Reseat / reset')}</option><option value="replace_gpu">{tx('更换 GPU', 'Replace GPU')}</option><option value="board_replacement">{tx('板卡更换确认', 'Board replacement confirmed')}</option><option value="replace_node">{tx('整机维修 / 更换', 'Node repair / replacement')}</option><option value="no_hardware_fault">{tx('排除硬件故障', 'No hardware fault')}</option></select></label>
          ...
          {selectedFeedbackAsset ? <p className="node-access-note">{tx('将请求离线获取：', 'Will request offline collection:')} {selectedFeedbackAsset.node_ip} · GPU {selectedFeedbackAsset.gpu_index} · {replacementFeedback ? tx('按故障时间回查历史 UUID，当前 UUID 仅作参考', 'resolve historical UUID at fault time; current UUID is reference only') : (selectedFeedbackAsset.gpu_uuid || 'UUID UNKNOWN')} · T-{feedbackForm.pre_window_hours}h / T+{feedbackForm.post_window_hours}h</p> : null}
