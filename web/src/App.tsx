...
type BaselineEvaluationReport = { matrix_key: string; by_test_event_type: Record<string, BaselineMetrics>; by_test_driver_version: Record<string, BaselineMetrics>; by_test_label_source: Record<string, BaselineMetrics>; by_test_hardware_certainty: Record<string, BaselineMetrics>; by_test_rule_version: Record<string, BaselineMetrics> };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="SUPERVISED TRAINING MATRIX" title={tx('监督训练矩阵', 'Supervised Training Matrix')} action={<div className="table-actions"><Badge value="LEAKAGE AUDITED" kind="healthy" /><button className="link" type="button" disabled={trainingMatrixStarting || latestControlFeatureBuild?.status !== 'completed' || latestTrainingMatrixBuild?.status === 'queued' || latestTrainingMatrixBuild?.status === 'running'} onClick={() => void buildTrainingMatrix()}>{trainingMatrixStarting ? tx('启动中…', 'Starting…') : tx('构建训练矩阵', 'Build matrix')}</button></div>} />
      ...
      <Card className="span-12">
        ...
        {baselineEvaluationReport && Object.keys(baselineEvaluationReport.by_test_event_type || {}).length > 0 ? <div className="table-wrap"><table><thead><tr><th>{tx('故障类型', 'Fault type')}</th><th>{tx('测试样本', 'Test samples')}</th><th>ROC-AUC</th><th>PR-AUC</th><th>{tx('精确率', 'Precision')}</th><th>{tx('召回率', 'Recall')}</th></tr></thead><tbody>{Object.entries(baselineEvaluationReport.by_test_event_type).sort(([left], [right]) => left.localeCompare(right)).map(([eventType, metrics]) => <tr key={eventType}><td><b>{eventType}</b></td><td>{metrics.count}<small>{metrics.positive} POS / {metrics.control} CTRL</small></td><td>{metrics.roc_auc.toFixed(3)}</td><td>{metrics.pr_auc.toFixed(3)}</td><td>{metrics.precision.toFixed(3)}</td><td>{metrics.recall.toFixed(3)}</td></tr>)}</tbody></table></div> : null}
      </Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.0',
    status: tx('分层离线评估', 'STRATIFIED OFFLINE EVALUATION'),
    desc: tx(
