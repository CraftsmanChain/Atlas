...
type BaselineEvaluationReport = { matrix_key: string; horizons: { horizon_minutes: number; test_by_event_type: Record<string, BaselineMetrics>; test_uncertainty?: BaselineUncertainty }[]; by_test_event_type: Record<string, BaselineMetrics>; by_test_driver_version: Record<string, BaselineMetrics>; by_test_label_source: Record<string, BaselineMetrics>; by_test_hardware_certainty: Record<string, BaselineMetrics>; by_test_rule_version: Record<string, BaselineMetrics> };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        {baselineEvaluationReport && baselineEvaluationReport.horizons?.length > 0 ? <div className="table-wrap"><table><thead><tr><th>{tx('故障类型', 'Fault type')}</th><th>{tx('预测窗口', 'Horizon')}</th><th>{tx('测试样本', 'Test samples')}</th><th>ROC-AUC / 95% CI</th><th>PR-AUC / 95% CI</th><th>{tx('稳定性', 'Stability')}</th><th>{tx('精确率', 'Precision')}</th><th>{tx('召回率', 'Recall')}</th></tr></thead><tbody>{baselineEvaluationReport.horizons.flatMap(horizon => Object.entries(horizon.test_by_event_type || {}).map(([eventType, metrics]) => ({ eventType, horizon: horizon.horizon_minutes, metrics, uncertainty: horizon.test_uncertainty }))).sort((left, right) => left.eventType.localeCompare(right.eventType) || left.horizon - right.horizon).map(({ eventType, horizon, metrics, uncertainty }) => <tr key={`${eventType}-${horizon}`}><td><b>{eventType}</b></td><td>{horizon}m</td><td>{metrics.count}<small>{metrics.positive} POS / {metrics.control} CTRL</small></td><td>{metrics.roc_auc.toFixed(3)}<small>{uncertainty ? `${uncertainty.roc_auc_lower.toFixed(3)}–${uncertainty.roc_auc_upper.toFixed(3)}` : 'CI PENDING'}</small></td><td>{metrics.pr_auc.toFixed(3)}<small>{uncertainty ? `${uncertainty.pr_auc_lower.toFixed(3)}–${uncertainty.pr_auc_upper.toFixed(3)} · NULL ${uncertainty.null_pr_auc.toFixed(3)}` : 'CI PENDING'}</small></td><td><Badge value={(uncertainty?.status || 'not_audited').toUpperCase()} kind={uncertainty?.status === 'candidate_signal' ? 'healthy' : uncertainty?.status === 'inverse_signal' ? 'danger' : 'warning'} /></td><td>{metrics.precision.toFixed(3)}</td><td>{metrics.recall.toFixed(3)}</td></tr>)}</tbody></table></div> : null}
      </Card>
