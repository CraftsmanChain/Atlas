  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'feedback-pack' ? <Card id="prediction-feedback-package" className="span-12">
        ...
        <div className="table-wrap"><table><thead><tr><th>{tx('故障对象', 'Fault target')}</th><th>{tx('故障类型 / 时间', 'Fault type / time')}</th><th>{tx('窗口', 'Window')}</th><th>{tx('采集状态', 'Collection status')}</th><th>{tx('身份解析', 'Identity resolution')}</th><th>{tx('维修结论', 'Repair result')}</th><th>{tx('反馈人', 'Operator')}</th></tr></thead><tbody>{faultFeedbackRequests.map(row => <tr key={row.id}><td><b>{row.node_ip} · GPU {row.gpu_index}</b><small>{row.gpu_uuid || tx('等待历史 UUID', 'HISTORICAL UUID PENDING')} · {row.reported_gpu_uuid ? `${tx('当前/上报', 'reported')} ${row.reported_gpu_uuid}` : (row.model_name || 'MODEL UNKNOWN')}</small></td><td><b>{row.fault_type}</b><small>{time(row.fault_occurred_at, lang)}</small></td><td><strong>T-{row.pre_window_hours}h / T+{row.post_window_hours}h</strong><small>{row.history_pack_scope}</small></td><td><Badge value={row.history_pack_status.toUpperCase()} kind={row.history_pack_status.includes('queued') ? 'warning' : row.history_pack_status.includes('ready') ? 'healthy' : 'info'} /><small>{(row.blocking_reasons || []).join(' · ') || row.history_pack_sha256 || row.status}</small></td><td><Badge value={(row.identity_resolution_status || 'current_identity_selected').toUpperCase()} kind={row.identity_resolution_status === 'requires_historical_identity_at_fault_time' ? 'warning' : 'healthy'} /><small>{row.identity_resolution_note || tx('当前身份可用于故障窗口', 'Current identity usable for fault window')}</small></td><td><b>{row.repair_action || '—'}</b><small>{row.hardware_replaced ? tx('硬件已更换/维修', 'Hardware repaired/replaced') : tx('未确认更换硬件', 'No replacement confirmed')}</small></td><td><b>{row.operator}</b><small>{time(row.created_at, lang)} · {row.training_eligible ? 'TRAINING' : 'NO TRAINING'}</small></td></tr>)}</tbody></table>{faultFeedbackRequests.length === 0 && <Empty tx={tx} title={tx('还没有人工反馈产生的数据包请求', 'No data-pack requests from human feedback yet')} />}</div>
      </Card> : null}
