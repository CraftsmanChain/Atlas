...
type BaselineEvaluationReport = { matrix_key: string; horizons: { horizon_minutes: number; test_by_event_type: Record<string, BaselineMetrics> }[]; by_test_event_type: Record<string, BaselineMetrics>; by_test_driver_version: Record<string, BaselineMetrics>; by_test_label_source: Record<string, BaselineMetrics>; by_test_hardware_certainty: Record<string, BaselineMetrics>; by_test_rule_version: Record<string, BaselineMetrics> };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, accuracyResponse, outcomesResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/accuracy'), fetch('/api/v1/prediction/outcomes'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !accuracyResponse.ok || !outcomesResponse.ok || !historyResponse.ok || !backfillResponse.ok || !candidatesResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${accuracyResponse.status}/${outcomesResponse.status}/${historyResponse.status}/${backfillResponse.status}/${candidatesResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, accuracyPayload, outcomesPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), accuracyResponse.json(), outcomesResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="SCOPED LOGISTIC BASELINE" title={tx('质量门类型专用基线', 'Readiness-gated Scoped Baseline')} action={<div className="table-actions"><Badge value="NO ONLINE PROBABILITY" kind="info" /><button className="link" type="button" disabled={baselineModelStarting || latestTrainingMatrixBuild?.status !== 'completed' || latestTrainingMatrixBuild?.version !== 'gpu-supervised-training-matrix-v3' || !matrixReadinessReport?.strata.some(item => item.status === 'exploratory_ready') || latestBaselineModelBuild?.status === 'queued' || latestBaselineModelBuild?.status === 'running'} onClick={() => void trainBaselineModels()}>{baselineModelStarting ? tx('训练中…', 'Training…') : tx('训练就绪分层', 'Train ready scope')}</button></div>} />
        ...
        {baselineEvaluationReport && baselineEvaluationReport.horizons?.length > 0 ? <div className="table-wrap"><table><thead><tr><th>{tx('故障类型', 'Fault type')}</th><th>{tx('预测窗口', 'Horizon')}</th><th>{tx('测试样本', 'Test samples')}</th><th>ROC-AUC</th><th>PR-AUC</th><th>{tx('精确率', 'Precision')}</th><th>{tx('召回率', 'Recall')}</th></tr></thead><tbody>{baselineEvaluationReport.horizons.flatMap(horizon => Object.entries(horizon.test_by_event_type || {}).map(([eventType, metrics]) => ({ eventType, horizon: horizon.horizon_minutes, metrics }))).sort((left, right) => left.eventType.localeCompare(right.eventType) || left.horizon - right.horizon).map(({ eventType, horizon, metrics }) => <tr key={`${eventType}-${horizon}`}><td><b>{eventType}</b></td><td>{horizon}m</td><td>{metrics.count}<small>{metrics.positive} POS / {metrics.control} CTRL</small></td><td>{metrics.roc_auc.toFixed(3)}</td><td>{metrics.pr_auc.toFixed(3)}</td><td>{metrics.precision.toFixed(3)}</td><td>{metrics.recall.toFixed(3)}</td></tr>)}</tbody></table></div> : null}
      </Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.20.0',
    status: tx('质量门类型专用基线', 'READINESS-GATED SCOPED BASELINE'),
    desc: tx(
