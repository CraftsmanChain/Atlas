...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness')]);
        if (!overviewResponse.ok || !readinessResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}`);
        const [overviewPayload, readinessPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json()]);
        if (!cancelled) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {[[tx('当前 GPU', 'Current GPUs'), summary?.total ?? '—', tx('进入就绪评估', 'evaluated for readiness')], [tx('数据集就绪', 'Dataset Ready'), summary?.ready_for_dataset ?? '—', tx('仅代表可构建样本', 'eligible for dataset only')], [tx('被质量门阻断', 'Blocked by Gates'), summary?.blocked ?? '—', tx('不进入训练或推理', 'excluded from training/inference')], [tx('预测概率', 'Probabilities'), prediction?.probability_emitted ? tx('已启用', 'ENABLED') : tx('未启用', 'DISABLED'), tx('模型尚未发布', 'no released model')]].map(([label, value, note]) => <Card className="metric prediction-metric" key={String(label)}><BrainCircuit size={17} /><span>{label}</span><strong>{value}</strong><small>{note}</small></Card>)}
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.1.0',
    status: tx('GPU 预测框架就绪', 'GPU PREDICTION FRAMEWORK'),
    desc: tx(
      '建立 GPU 优先并可扩展到服务器、存储和网络硬件的预测契约，覆盖 1h/6h/24h 时间窗、模型注册、标签质量、point-in-time 数据门和 shadow 上线约束；当前不输出故障概率。',
      'Establish GPU-first prediction contracts extensible to server, storage, and network hardware, covering 1h/6h/24h horizons, model registration, label quality, point-in-time data gates, and shadow release controls. No failure probability is emitted yet.',
    ),
    history: [
