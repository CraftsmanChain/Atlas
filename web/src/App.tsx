...
type GPUHealthScore = { id: number; gpu_asset_id: number; gpu_uuid: string; node_ip: string; gpu_index: number; model_name: string; score: number | null; level: string; data_confidence: string; stability_score: number; memory_score: number; thermal_score: number; power_score: number; interconnect_score: number; performance_score: number; evidence: string[]; rule_version: string; evaluated_at: string };
...
function Gpus({ tx, view, assets, models, loading, inventoryError, healthScores, healthSummary, lang }: { tx: Tx; view: string; assets: GPUAsset[]; models: { model: string; short: string; count: number }[]; loading: boolean; inventoryError: boolean; healthScores: GPUHealthScore[]; healthSummary: HealthSummary | null; lang: string }) {
  ...
  if (view === 'health') {
    const risky = healthScores.filter(x => x.score == null || x.level !== 'healthy').sort((a, b) => {
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.3.0', status: tx('时效基线完成', 'FRESHNESS BASELINE'), desc: tx('识别 Target 异常、指标缺失或陈旧、覆盖不足、身份冲突与采集链路问题，并分别标识接收、资产与健康数据的新鲜度，不依赖节点登录权限。', 'Detect target failures, missing or stale metrics, coverage gaps, identity conflicts and collection-path issues, with separate freshness states for ingestion, inventory and health data, without requiring node access.'), history: [tx('v0.3.0 · 统一数据时效 API、SLA 边界和 8077 源时间标识', 'v0.3.0 · unified freshness API, SLA boundaries and source-time indicator on 8077'), tx('v0.2.0 · 原因分类、节点抑制与同步审计', 'v0.2.0 · reason classification, node suppression and sync audit'), tx('v0.1.0 · 四类 Target 覆盖矩阵', 'v0.1.0 · four-target coverage matrix')] },
    { id: 'health', name: tx('硬件健康评分', 'Hardware Health Scoring'), version: 'v1.0.0', status: tx('GPU 基线完成', 'GPU BASELINE'), desc: tx('以可解释规则、六个健康领域和数据置信度输出 GPU 当前健康状态，并为其他硬件评分预留统一模型。', 'Score current GPU health using explainable rules, six health domains and data confidence, with a common model for future hardware types.'), history: [tx('v1.0.0 · GPU 六领域规则评分与 A–D 置信度', 'v1.0.0 · six-domain GPU rules and A–D confidence')] },
    ...
    { id: 'feature', name: tx('统一特征目录', 'Unified Feature Catalog'), version: 'v1.0.0', status: tx('核心基线完成', 'CORE BASELINE'), desc: tx('为健康评分、PyOD、风险排序、监督预测和性能衰减提供版本化特征契约、型号能力、时间与缺失语义、质量、血缘和 owner。', 'Provide versioned feature contracts, model capabilities, time and missing semantics, quality, lineage and ownership for health scoring, PyOD, risk ranking, supervised prediction and degradation detection.'), history: [tx('v1.0.0 · 注册/读取 API、20 个健康核心特征和 snapshot 版本 manifest', 'v1.0.0 · registration/read APIs, 20 core health features and snapshot version manifests')] },
