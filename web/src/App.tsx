...
type NodeAccessCommand = { id: string; category: string; approval_class: string; purpose: LocalizedText; preview: string };
type NodeCredentialStatus = { id: string; priority: number; username: string; auth_type: string; secret_provider: string; enabled: boolean; secret_available: boolean; status: string };
type NodeAccessOverview = {
  skill_id: string; skill_version: string; status: string; enabled: boolean; execution_enabled: boolean;
  no_arbitrary_shell: boolean; no_change_executed: boolean;
  ...
  credential_profiles: NodeCredentialStatus[]; commands: NodeAccessCommand[]; boundaries: LocalizedText[]; generated_at: string;
};
...
function Quality({ tx, view, targets, summary, issueSummary, inventoryError, syncRuns, assetChanges, telemetry, telemetrySummary, nodeAccess, openIssues, lang }: { tx: Tx; view: string; targets: CollectorTarget[]; summary: FleetSummary | null; issueSummary: IssueSummary | null; inventoryError: boolean; syncRuns: SyncRun[]; assetChanges: AssetChange[]; telemetry: TelemetryQualityItem[]; telemetrySummary: TelemetryQualitySummary | null; nodeAccess: NodeAccessOverview | null; openIssues: () => void; lang: string }) {
  ...
  if (view === 'node-access') {
    ...
    const statusKind = nodeAccess?.status === 'ready_no_transport' ? 'healthy' : nodeAccess?.status === 'credentials_missing' ? 'warning' : 'neutral';
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.1.0'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('所有节点信息收集都必须使用已注册的 Skill 命令 ID、固定参数和资源预算。v0.1 已完成安全契约与状态可见性，但未启用远程连接或命令执行。', 'All node evidence collection must use registered Skill command IDs, fixed parameters, and resource budgets. v0.1 delivers the safety contract and visible status, while remote connections and command execution remain disabled.')}</p></div></div>
        <div className="node-access-badges"><Badge value={(nodeAccess?.status || 'unavailable').toUpperCase()} kind={statusKind} /><Badge value="EXECUTION DISABLED" kind="info" /><Badge value="NO CHANGE EXECUTED" kind="healthy" /></div>
      </Card>
      {[
      ...
      <Card className="span-12">
        <CardHead code="CREDENTIAL ROTATION" title={tx('凭据引用与尝试顺序', 'Credential References & Attempt Order')} action={<Badge value={`${nodeAccess?.credential_profiles.length || 0} PROFILES`} kind={nodeAccess?.credential_profiles.some(profile => profile.secret_available) ? 'healthy' : 'neutral'} />} />
        <p className="node-access-note">{tx('平台仅展示凭据配置的安全元数据，不返回 secret_ref、密码或私钥。认证拒绝时按优先级尝试下一配置；网络或主机指纹异常立即停止；全部失败进入 credential_exhausted 并要求告警。', 'Only safe credential metadata is exposed. secret_ref, passwords, and private keys are never returned. Authentication rejection advances to the next priority; network or host-identity failures stop immediately; exhausting all profiles raises credential_exhausted and requires an alert.')}</p>
        <div className="table-wrap"><table className="node-access-table"><thead><tr><th>{tx('顺序', 'Order')}</th><th>PROFILE</th><th>{tx('账号', 'Username')}</th><th>{tx('认证方式', 'Auth type')}</th><th>{tx('密钥来源', 'Secret provider')}</th><th>{tx('状态', 'Status')}</th></tr></thead><tbody>{nodeAccess?.credential_profiles.map(profile => <tr key={profile.id}><td><code>{profile.priority}</code></td><td><b>{profile.id}</b></td><td><code>{profile.username}</code></td><td>{profile.auth_type}</td><td><Badge value={profile.secret_provider.toUpperCase()} kind="info" /></td><td><Badge value={profile.status.toUpperCase()} kind={profile.status === 'ready' ? 'healthy' : profile.status === 'secret_unavailable' ? 'warning' : 'neutral'} /></td></tr>)}</tbody></table>{!nodeAccess?.credential_profiles.length && <Empty tx={tx} title={tx('尚未配置凭据引用', 'No credential references configured')} />}</div>
      </Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'node-access', name: tx('节点证据 Skill', 'Node Evidence Skill'), version: 'v0.1.0', status: tx('安全底座完成', 'SAFETY BASELINE'), desc: tx('以版本化 Skill 统一声明节点信息采集、命令白名单、资源预算、凭据引用轮换和人工审批边界。当前仅提供安全契约与状态接口，不建立远程连接。', 'A versioned Skill defines node evidence collection, registered commands, resource budgets, credential-reference rotation, and human-approval boundaries. The current release exposes only its safety contract and status API, with no remote connection.'), history: [tx('v0.1.0 · Skill 契约、只读/审批命令分级、凭据引用轮换状态机与状态页面', 'v0.1.0 · Skill contract, read-only/approval command classes, credential-reference rotation state machine, and status UI'), tx('v0.1.0 · 禁止明文凭据、任意 Shell、远程连接和执行 API', 'v0.1.0 · plaintext credentials, arbitrary shell, remote connections, and execution APIs are prohibited')] },
