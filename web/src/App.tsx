...
type ModelRaceComparison = { version: string; reference_build_id: number; build_ids: number[]; source_matrix_build_id: number; source_training_matrix_key: string; matrix_sha256: string; comparison_sha256: string; builds: Array<{ build_id: number; model_key: string; version: string; algorithm: string; macro_test: BaselineMetrics; stable_count: number; candidate_count: number }>; deltas_vs_reference: Array<{ build_id: number; reference_build_id: number; roc_auc: number; pr_auc: number; precision: number; recall: number }> };
  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        {modelRaceComparison ? <div className="prediction-contract"><span><b>IMMUTABLE RACE</b>REF #{modelRaceComparison.reference_build_id} · {modelRaceComparison.build_ids.length} BUILDS</span><span><b>MATRIX</b>#{modelRaceComparison.source_matrix_build_id} · {modelRaceComparison.matrix_sha256.slice(0, 12)}</span>{modelRaceComparison.builds.map(build => <span key={build.build_id}><b>#{build.build_id} · {build.algorithm}</b>ROC {build.macro_test.roc_auc.toFixed(3)} · PR {build.macro_test.pr_auc.toFixed(3)} · P/R {build.macro_test.precision.toFixed(3)}/{build.macro_test.recall.toFixed(3)}</span>)}<span><b>COMPARISON SHA</b>{modelRaceComparison.comparison_sha256}</span></div> : null}
