...
type ValidationReadinessReport = { version: string; framework_version: string; mode: string; status: string; label_gate_status: string; label_manifest_sha256: string; outcome_maturity: { total: number; matured: number; pending: number; censored: number; matured_ratio: number; probability_scored: number; node_eligible: number }; challenger_status: string; seven_day: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; ranking_at_k?: RankingAtK[] }>; blocking_reasons: string[]; recommended_next_run: string[]; generated_at: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('标签门', 'Label gate')}</b><strong>{validationReadiness.label_gate_status.toUpperCase()}</strong><small>{validationReadiness.label_manifest_sha256.slice(0, 16) || 'NO MANIFEST SHA'}</small></div>
          ...
          <div><b>{tx('7 天 Challenger', '7d challenger')}</b><strong>{validationReadiness.challenger_status.toUpperCase()}</strong><small>{validationReadiness.seven_day[0]?.rows || 0} ROWS · {validationReadiness.seven_day[0]?.positives || 0} POS</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.9.0',
    status: tx('离线验证就绪门', 'OFFLINE VALIDATION GATE'),
    desc: tx(
