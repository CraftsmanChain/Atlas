...
type AccuracyMetrics = { tp: number; fp: number; fn: number; tn: number; evaluated: number; precision?: number; recall?: number; specificity?: number; false_positive_rate?: number; false_negative_rate?: number; accuracy?: number; ranking_at_k?: RankingAtK[] };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  const rankingRows = (metrics?: AccuracyMetrics) => (metrics?.ranking_at_k || []).filter(row => row.eligible > 0);
  const RankingTable = ({ label, metrics }: { label: string; metrics?: AccuracyMetrics }) => {
    const rows = rankingRows(metrics);
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        <div className="prediction-ranking-grid">
          <RankingTable label={tx('规则排序', 'Rule Ranking')} metrics={predictionAccuracy?.rule} />
          <RankingTable label={tx('人工最终排序', 'Human-final Ranking')} metrics={predictionAccuracy?.final} />
        </div>
