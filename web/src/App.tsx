...
type HistoricalFaultCandidate = { id: number; candidate_key: string; source_key: string; gpu_uuid: string; node_ip: string; hostname: string; model_name: string; pci_bus_id: string; event_type: string; event_code: string; event_message: string; severity: string; quality_tier: string; operational_priority: string; hardware_certainty: string; training_disposition: string; recommended_action: string; recovery_aware: boolean; review_status: string; source_metric: string; source_alert_name: string; signal_samples: number; onset_at: string; detection_window_end_at: string };
type HistoricalCandidateSummary = { total: number; pending_review: number; by_event_code: Record<string, number>; by_quality_tier: Record<string, number>; by_operational_priority: Record<string, number>; by_hardware_certainty: Record<string, number>; by_training_disposition: Record<string, number>; by_model: Record<string, number>; earliest_onset_at?: string; latest_onset_at?: string };
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.5.0',
    status: tx('故障语义与训练队列', 'FAULT SEMANTICS & TRAINING COHORT'),
    desc: tx(
      '在部署节点本地从历史告警和确定性指标重建 GPU 故障候选，分离处置优先级、硬件确定性与训练用途，并定义匹配正常对照；候选不会自动成为确认训练标签。',
      'Reconstruct GPU fault candidates locally from historical alerts and deterministic metrics, separate operational priority, hardware certainty and training use, and define matched healthy controls. Candidates never become confirmed labels automatically.',
    ),
    history: [
