...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {shadowModelCandidates.length > 0 ? <Card className="span-12"><CardHead code="SHADOW MODEL REGISTRY" title={tx('影子候选模型', 'Shadow Model Candidates')} action={<Badge value="SCORING DISABLED" kind="info" />} /><p className="node-access-note">{tx('候选注册只确认模型产物可追溯且离线门禁通过，不等同于线上发布。在线特征必须与训练特征逐列等价，之后才允许生成只读影子预测记录。', 'Candidate registration confirms artifact provenance and offline gates only; it is not an online release. Online features must match training features column by column before read-only shadow prediction records may be generated.')}</p><div className="table-wrap"><table><thead><tr><th>{tx('故障 / 型号', 'Fault / Model')}</th><th>{tx('窗口', 'Horizon')}</th><th>{tx('状态', 'Status')}</th><th>{tx('阈值', 'Threshold')}</th><th>{tx('模型来源', 'Model Provenance')}</th><th>{tx('注册门', 'Registry Gate')}</th></tr></thead><tbody>{shadowModelCandidates.map(model => <tr key={`${model.model_key}-${model.version}`}><td><b>{model.scope_event_type || model.task}</b><small>{model.scope_model_name || 'ALL MODELS'}</small></td><td>{model.horizon_minutes}m</td><td><Badge value={model.status.toUpperCase()} kind="healthy" /><small>{model.mode.toUpperCase()} · NO ACTION</small></td><td>{model.decision_threshold?.toFixed(3) || '—'}</td><td><b>BUILD #{model.source_baseline_build_id || '—'}</b><small>{model.version}</small><small>{model.artifact_sha256?.slice(0, 16) || 'SHA256 MISSING'}</small></td><td><code>{model.registry_gate_version || '—'}</code><small>{model.feature_contract_version}</small></td></tr>)}</tbody></table></div></Card> : null}
      <Card className="span-12">
