...
type NodeAccessCommand = { id: string; category: string; approval_class: string; planning_status: string; purpose: LocalizedText; preview: string };
...
type NodeExecutionPlan = { plan_id: string; skill_id: string; skill_version: string; node_ip: string; status: string; approval_class: string; commands: NodeAccessCommand[]; budget: NodeAccessOverview['budget']; created_at: string; expires_at: string; execution_enabled: boolean; no_command_executed: boolean };
type NodeAccessOverview = {
  skill_id: string; skill_version: string; status: string; enabled: boolean; execution_enabled: boolean;
  no_arbitrary_shell: boolean; no_change_executed: boolean; encryption_ready: boolean; management_ready: boolean; secure_write_only: boolean; insecure_http_allowed: boolean; connectivity_check_enabled: boolean; known_hosts_ready: boolean; plan_preview_enabled: boolean;
...
function Quality({ tx, view, targets, summary, issueSummary, inventoryError, syncRuns, assetChanges, telemetry, telemetrySummary, nodeAccess, reloadNodeAccess, openIssues, lang }: { tx: Tx; view: string; targets: CollectorTarget[]; summary: FleetSummary | null; issueSummary: IssueSummary | null; inventoryError: boolean; syncRuns: SyncRun[]; assetChanges: AssetChange[]; telemetry: TelemetryQualityItem[]; telemetrySummary: TelemetryQualitySummary | null; nodeAccess: NodeAccessOverview | null; reloadNodeAccess: () => void; openIssues: () => void; lang: string }) {
  ...
  const [planNodeIP, setPlanNodeIP] = useState('');
  const [planCommandIDs, setPlanCommandIDs] = useState<string[]>([]);
  const [executionPlan, setExecutionPlan] = useState<NodeExecutionPlan | null>(null);
  ...
  const togglePlanCommand = (commandID: string) => setPlanCommandIDs(current => current.includes(commandID) ? current.filter(id => id !== commandID) : [...current, commandID]);
  const buildExecutionPlan = async () => {
    setPlanBuilding(true); setPlanError(''); setExecutionPlan(null);
    try {
      const response = await fetch('/api/v1/node-access/plans', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Atlas-Admin-Token': credentialAdminToken },
        body: JSON.stringify({ node_ip: planNodeIP, command_ids: planCommandIDs }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error?.message || `HTTP ${response.status}`);
      setExecutionPlan(payload.data || null);
    } catch (reason) {
      setPlanError(reason instanceof Error ? reason.message : tx('计划生成失败', 'Plan generation failed'));
    } finally { setPlanBuilding(false); }
  };
  ...
  if (view === 'node-access') {
    ...
    const planReady = readOnly.filter(command => command.planning_status === 'ready');
    ...
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.3.1'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('v0.3.1 在 known-host SSH 仅认证基线上增加注册只读命令的计划预览；预览不创建 session、不执行命令，实际采集仍需后续受控执行器。', 'v0.3.1 adds registered read-only command plan previews to the known-host authentication baseline. A preview opens no session and executes no command; actual collection still requires the future controlled runner.')}</p></div></div>
      ...
      <Card className="span-12 plan-preview-card">
        <CardHead code="EXECUTION PLAN / PREVIEW ONLY" title={tx('只读证据采集计划', 'Read-only Evidence Collection Plan')} action={<div className="table-actions"><Badge value="NO EXECUTION" kind="healthy" /><Badge value={`${planReady.length} READY`} kind="info" /></div>} />
        <p className="node-access-note">{tx('先生成受管节点、注册命令和资源预算的不可执行预览。当前 API 不创建 SSH session、不下发命令；需要参数或人工审批的命令不能加入计划。', 'Generate a non-executable preview of the managed node, registered commands, and resource budget. The current API opens no SSH session and dispatches no command; parameterized or approval-required commands cannot enter a plan.')}</p>
        <div className="plan-builder">
          <label>{tx('节点 IP', 'Node IP')}<input value={planNodeIP} onChange={event => setPlanNodeIP(event.target.value)} placeholder="10.114.4.25" /></label>
          <div className="plan-command-options">{planReady.map(command => <label key={command.id} className={planCommandIDs.includes(command.id) ? 'selected' : ''}><input type="checkbox" checked={planCommandIDs.includes(command.id)} onChange={() => togglePlanCommand(command.id)} /><span><b>{command.id}</b><small>{local(command.purpose)}</small></span></label>)}</div>
          <button className="primary-action node-action" type="button" onClick={() => void buildExecutionPlan()} disabled={planBuilding || !credentialTransportReady || !credentialAdminToken || !nodeAccess?.plan_preview_enabled || !planNodeIP || planCommandIDs.length === 0}><ClipboardList size={15} />{planBuilding ? tx('生成中', 'Building') : tx('生成计划预览', 'Build Plan Preview')}</button>
        </div>
        {planError && <p className="form-error">{planError}</p>}
        {executionPlan && <div className="plan-result"><header><div><code>{executionPlan.plan_id}</code><b>{executionPlan.node_ip}</b></div><Badge value={executionPlan.status.toUpperCase()} kind="info" /></header><div>{executionPlan.commands.map(command => <span key={command.id}><CheckCircle2 size={13} />{command.id}</span>)}</div><small>{tx('每节点命令上限', 'Per-node command limit')} {executionPlan.budget.max_commands_per_node} · {tx('命令超时', 'Command timeout')} {executionPlan.budget.command_timeout_seconds}s · {tx('过期', 'Expires')} {time(executionPlan.expires_at, lang)} · NO COMMAND EXECUTED</small></div>}
      </Card>
      <Card className="span-7 node-command-card">
        <CardHead code="AUTO / READ-ONLY" title={tx('可自动执行的只读采集', 'Auto-allowed Read-only Collection')} action={<Badge value={`${readOnly.length} COMMANDS`} kind="healthy" />} />
        <div className="node-command-list">{readOnly.map(command => <article key={command.id}><header><code>{command.id}</code><Badge value={command.category.toUpperCase()} kind="info" /></header><b>{local(command.purpose)}</b><small>{command.preview}</small></article>)}</div>
      </Card>
      <Card className="span-5 node-command-card approval">
        <CardHead code="HUMAN APPROVAL" title={tx('必须人工确认的操作', 'Human Approval Required')} action={<Badge value={`${approvalRequired.length} COMMANDS`} kind="warning" />} />
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'node-access', name: tx('节点证据 Skill', 'Node Evidence Skill'), version: 'v0.3.1', status: tx('只读计划预览', 'READ-ONLY PLAN PREVIEW'), desc: tx('对 Atlas 非退役受管节点执行 known-host SSH 仅认证，并可按注册命令、资源预算生成不可执行的只读采集计划预览。预览和认证均不创建 session、不执行命令。', 'Perform known-host SSH authentication checks for non-retired Atlas-managed assets and generate non-executable read-only collection plan previews from registered commands and resource budgets. Neither previews nor authentication checks open a session or execute commands.'), history: [tx('v0.3.1 · 增加受管节点、注册命令和资源预算的不可执行计划预览', 'v0.3.1 · added non-executable plan previews for managed nodes, registered commands, and resource budgets'), tx('v0.3.0 · 受管资产限制、known-host 校验、仅认证 API、审计记录和 access 问题自动开闭', 'v0.3.0 · managed-asset restriction, known-host verification, authentication-only API, audit records, and automatic access issue lifecycle'), tx('v0.2.1 · 增加默认关闭的 HTTP 兼容开关、API 状态与页面风险提示', 'v0.2.1 · added a default-off HTTP compatibility switch, API status, and UI risk warning'), tx('v0.2.0 · 页面加密录入、密文落库、管理口令写保护、掩码列表与认证前解密', 'v0.2.0 · encrypted page entry, ciphertext persistence, management-token write protection, masked lists, and pre-authentication decryption'), tx('v0.1.0 · Skill 契约、只读/审批命令分级、凭据引用轮换状态机与状态页面', 'v0.1.0 · Skill contract, read-only/approval command classes, credential-reference rotation state machine, and status UI')] },
