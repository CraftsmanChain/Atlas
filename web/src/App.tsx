...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json()]);
        if (!cancelled) {
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.2.0',
    status: tx('训练数据与标签治理', 'TRAINING DATA & LABEL GOVERNANCE'),
    desc: tx(
      '在 GPU 优先预测契约上增加一年在线历史保护、幂等故障标签台账和人工确认升级；规则事件保留为代理标签，只有完整硬件处置记录可成为确认标签。当前仍不输出故障概率。',
      'Add one-year online history protection, an idempotent failure-label ledger, and human-confirmation upgrades to the GPU-first prediction contract. Rule events remain proxy labels and only complete hardware resolutions become confirmed labels. No failure probability is emitted yet.',
    ),
    history: [
