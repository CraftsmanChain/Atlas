...
type HeaRankChallengerReport = { version: string; framework_version: string; mode: string; status: string; confidence_status: string; target_horizon_minutes: number; minimum_seven_day_rows: number; minimum_seven_day_nodes: number; minimum_seven_day_positives: number; sample_summary: { total: number; matured: number; pending: number; censored: number; matured_ratio: number; probability_scored: number; node_eligible: number }; all_matured: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; ranking_at_k?: RankingAtK[] }>; seven_day: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; ranking_at_k?: RankingAtK[] }>; blocking_reasons: string[]; interpretation: string[]; recommended_next_run: string[]; generated_at: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {heaRankChallenger ? <Card className="span-12">
        ...
        <p className="node-access-note">{tx('这是 HeaRank 思路的离线验证入口，不是已发布模型。当前只用成熟 shadow outcome 对比四种节点级排序信号：Logistic 概率、历史故障次数、90 天半衰期的历史故障新近度加权和阈值二值信号。历史基线仅使用预测截点前已关闭的 outcome 窗口，避免未来标签泄漏；没有足够 7 天成熟样本时保持阻断。', 'This is the offline validation entry for the HeaRank idea, not a released model. It compares four node-level ranking signals on mature shadow outcomes only: Logistic probability, prior failure count, a 90-day-half-life recency-weighted failure history, and threshold-binary score. Historical baselines use only outcome windows closed before each prediction cutoff to prevent future-label leakage, and remain blocked until enough mature 7-day samples exist.')}</p>
        <div className="issue-categories quality-ledger">
