...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="SCOPED LOGISTIC BASELINE" title={tx('质量门类型专用基线', 'Readiness-gated Scoped Baseline')} action={<div className="table-actions"><Badge value="NO ONLINE PROBABILITY" kind="info" /><button className="link" type="button" disabled={baselineModelStarting || latestTrainingMatrixBuild?.status !== 'completed' || latestTrainingMatrixBuild?.version !== 'gpu-supervised-training-matrix-v5' || !baselineTrainingEnabled || latestBaselineModelBuild?.status === 'queued' || latestBaselineModelBuild?.status === 'running'} onClick={() => void trainBaselineModels()}>{baselineModelStarting ? tx('训练中…', 'Training…') : tx('训练就绪分层', 'Train ready scope')}</button></div>} />
        <p className="node-access-note">{tx('只对通过数据充分性门的预测窗分别训练零外部依赖 Logistic Regression；按 horizon 仅用训练集过滤低覆盖/常量特征、排序效应并限制每个源指标最多 4 列，验证/测试标签不参与选择。XID、ECC、Row Remap 和 Reset 等已发生故障指标禁止入模；独立测试集必须通过稳定性、校准及 precision≥70%/recall≥50% 门禁，不发布线上概率。', 'Train a dependency-free Logistic Regression separately only for ready horizons. Per-horizon selection uses training data alone to remove sparse/constant features, rank effects, and cap each source metric at four columns; validation/test labels never select features. XID, ECC, row-remap, reset and other occurred-fault indicators are prohibited. The independent test must pass stability, calibration, precision ≥70%, and recall ≥50%; no online probability is released.')}</p>
