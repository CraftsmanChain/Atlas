...
function Quality({ tx, view, targets, summary, issueSummary, inventoryError, syncRuns, assetChanges, telemetry, telemetrySummary, openIssues, lang }: { tx: Tx; view: string; targets: CollectorTarget[]; summary: FleetSummary | null; issueSummary: IssueSummary | null; inventoryError: boolean; syncRuns: SyncRun[]; assetChanges: AssetChange[]; telemetry: TelemetryQualityItem[]; telemetrySummary: TelemetryQualitySummary | null; openIssues: () => void; lang: string }) {
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.7.0', status: tx('分类统计基线', 'CATEGORY STATISTICS'), desc: tx('联合识别每卡连续性和采集链路问题，并在系统数据质量页面按历史、已解决、遗留和当前检测统一统计。', 'Detect per-GPU continuity and collection-path issues, with unified discovered, resolved, remaining and active statistics on System Data Quality.'), history: [tx('v0.7.0 · 连续性问题接入系统数据质量分类统计和侧边栏遗留数', 'v0.7.0 · continuity findings joined System Data Quality category statistics and sidebar remaining count'), tx('v0.6.1 · Target 相对耗时改为审计项，不再单独产生问题', 'v0.6.1 · relative target duration became audit-only and no longer creates findings alone'), tx('v0.6.0 · 新增最大间隔、UUID 波动与 Target 抓取质量判定', 'v0.6.0 · maximum gap, UUID flaps and target scrape-quality decisions'), tx('v0.5.0 · 新增每卡 1h 样本存在率、当前样本年龄、连续性页面与问题闭环', 'v0.5.0 · per-GPU 1h presence, current sample age, continuity UI and issue lifecycle')] },
    ...
    { id: 'feature', name: tx('统一特征目录', 'Unified Feature Catalog'), version: 'v1.5.0', status: tx('CANARY 就绪', 'CANARY READY'), desc: tx('35 个在线健康特征、53 个源查询和 1 个 metric-family shadow recording-rule 特征；canary 仅限单节点，尚未发布到 Prometheus。', 'Thirty-five online health features, 53 source queries and one shadow metric-family recording-rule feature. The single-node canary is not yet published to Prometheus.'), history: [tx('v1.5.0 · metric-family 单节点 canary 配置、shadow 契约与范围保护', 'v1.5.0 · single-node metric-family canary config, shadow contract and scope guard'), tx('v1.4.0 · 新增最大间隔、UUID 波动和三项 Target 抓取结构特征', 'v1.4.0 · maximum gap, UUID flap and three target scrape features'), tx('v1.3.0 · 新增样本数、1h 存在率和当前样本年龄结构特征', 'v1.3.0 · sample count, 1h presence and current sample-age features'), tx('v1.2.0 · 27 个规范健康特征、45 个源查询，新增 Correctable Row Remap 增量', 'v1.2.0 · 27 canonical health features and 45 source queries with correctable-row-remap deltas')] },
