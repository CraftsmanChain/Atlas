  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {featureDriftReport ? <Card className="span-12">
        ...
        <p className="node-access-note">{tx('读取 baseline artifact 的模型特征列和最近一次 feature replay 结果，只判断是否具备逐特征 PSI/KS 的证据条件。当前没有持久化训练/线上逐列分布，因此 PSI/KS 明确标记为 pending，不伪造漂移结论。', 'Reads baseline artifact feature columns and the latest feature replay, and only decides whether evidence exists for per-feature PSI/KS. Because training/live per-column distributions are not persisted yet, PSI/KS is explicitly marked pending instead of fabricating drift conclusions.')}</p>
        <div className="issue-categories quality-ledger">
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.2',
    status: tx('逐特征漂移明细', 'PER-FEATURE DRIFT DETAILS'),
    desc: tx(
