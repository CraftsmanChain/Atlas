...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  const trainBaselineModels = async (algorithm: 'logistic_regression' | 'gradient_boosted_stumps' = 'logistic_regression') => {
  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="SCOPED MODEL RACE" title={tx('质量门类型专用模型赛马', 'Readiness-gated Scoped Model Race')} action={<div className="table-actions"><Badge value="NO ONLINE PROBABILITY" kind="info" /><button className="link" type="button" disabled={baselineModelStarting || latestTrainingMatrixBuild?.status !== 'completed' || latestTrainingMatrixBuild?.version !== 'gpu-supervised-training-matrix-v7' || !baselineTrainingEnabled || latestBaselineModelBuild?.status === 'queued' || latestBaselineModelBuild?.status === 'running'} onClick={() => void trainBaselineModels('logistic_regression')}>{baselineModelStarting ? tx('训练中…', 'Training…') : 'Logistic'}</button><button className="link" type="button" disabled={baselineModelStarting || latestTrainingMatrixBuild?.status !== 'completed' || latestTrainingMatrixBuild?.version !== 'gpu-supervised-training-matrix-v7' || !baselineTrainingEnabled || latestBaselineModelBuild?.status === 'queued' || latestBaselineModelBuild?.status === 'running'} onClick={() => void trainBaselineModels('gradient_boosted_stumps')}>Shallow GBDT</button></div>} />
        <p className="node-access-note">{tx('对通过数据充分性门的分层，使用同一不可变矩阵赛马 Logistic 与 Shallow GBDT。GBDT 仅用训练集做覆盖率、方差和指标族多样性筛选，并以 48 棵深度 1 树学习非线性阈值；验证/测试标签不参与选择。两者共享目标专用防泄漏审计、验证集校准/阈值和独立测试门禁。GBDT 当前仅生成离线 challenger，尚无运行时，绝不会注册为 shadow 或发布线上概率。', 'For data-ready strata, Logistic and Shallow GBDT race on the same immutable matrix. GBDT uses training-only coverage, variance, and metric-family diversity filtering, then 48 depth-1 trees to learn nonlinear thresholds; validation/test labels never select features. Both share target-specific leakage audits, validation-only calibration/thresholds, and independent-test gates. GBDT is currently an offline challenger with no runtime and can never register for shadow scoring or emit online probabilities.')}</p>
