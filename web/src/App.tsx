...
type HeaRankChallengerReport = { version: string; framework_version: string; mode: string; status: string; confidence_status: string; target_horizon_minutes: number; minimum_seven_day_rows: number; minimum_seven_day_nodes: number; minimum_seven_day_positives: number; sample_summary: { total: number; matured: number; pending: number; censored: number; matured_ratio: number; probability_scored: number; node_eligible: number }; all_matured: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; non_zero_score_rows: number; non_zero_score_nodes: number; ranking_at_k?: RankingAtK[] }>; seven_day: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; non_zero_score_rows: number; non_zero_score_nodes: number; ranking_at_k?: RankingAtK[] }>; blocking_reasons: string[]; interpretation: string[]; recommended_next_run: string[]; generated_at: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {heaRankChallenger ? <Card className="span-12">
        ...
        <div className="prediction-contract">{heaRankChallenger.seven_day.map(row => <span key={`${row.policy}-coverage`}><b>{row.policy.toUpperCase()}</b>{row.non_zero_score_rows} / {row.rows} {tx('有信号样本', 'SIGNAL ROWS')}<small>{row.non_zero_score_nodes} / {row.nodes} {tx('有信号节点', 'SIGNAL NODES')}</small></span>)}</div>
