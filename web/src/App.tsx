    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('特征漂移', 'Feature drift')}</b><strong>{validationReadiness.feature_drift_status.toUpperCase()}</strong><small>{validationReadiness.feature_drift_compared_features}/{validationReadiness.feature_drift_feature_columns} COMP · PSI {validationReadiness.feature_drift_maximum_psi.toFixed(3)} · KS {validationReadiness.feature_drift_maximum_ks.toFixed(3)} · REVIEW {validationReadiness.feature_drift_review_required_features}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.3',
    status: tx('特征漂移动态口径', 'DYNAMIC FEATURE DRIFT GUIDANCE'),
    desc: tx(
