...
const compactWorkflowStatus = (status?: string) => ({
  ...
  manual_feedback_ready_pending_control_sampling: 'MANUAL READY',
...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  if (isPredictionView) {
    ...
    const feedbackWorkflowSteps = [
      ...
      { key: 'prepare-training', label: tx('准备训练样本', 'Prepare samples'), meta: latestManualPreparation ? compactWorkflowStatus(latestManualPreparation.status) : isFeedbackFeatureDone(latestFeatureStatus) ? tx('等待准备', 'Ready') : tx('等待特征', 'Wait features'), state: latestManualPreparation ? latestManualPreparation.status.includes('blocked') ? 'blocked' : 'done' : isFeedbackFeatureDone(latestFeatureStatus) ? 'current' : 'locked', disabled: preparationBuilding || !isFeedbackFeatureDone(latestFeatureStatus) || Boolean(latestManualPreparation), onClick: () => void buildTrainingPreparation('manual-feedback') },
    ];
