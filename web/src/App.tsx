...
type HardwareFaultFeedbackRequest = { id: number; request_key: string; status: string; node_ip: string; gpu_uuid?: string; gpu_index: number; gpu_asset_id?: number; model_name?: string; fault_type: string; fault_occurred_at: string; pre_window_hours: number; post_window_hours: number; operator: string; description?: string; repair_action?: string; hardware_replaced: boolean; evidence_note?: string; training_eligible: boolean; history_pack_status: string; history_pack_scope: string; history_pack_sha256?: string; blocking_reasons: string[]; created_at: string; updated_at: string };
      ...
      try {
        ...
        const response = await fetch('/api/v1/prediction/hardware-fault-feedback', {
          ...
          body: JSON.stringify({
            ...
            gpu_uuid: selectedFeedbackAsset.gpu_uuid,
            ...
            hardware_replaced: feedbackForm.hardware_replaced,
    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'human-feedback' ? <Card id="prediction-human-feedback" className="span-12">
        ...
        <form className="settings-form fault-feedback-form" onSubmit={event => { event.preventDefault(); void submitHardwareFeedback(); }}>
          <label><span>{tx('故障机器 / GPU', 'Failed node / GPU')}<small>REQUIRED · GPU ASSET</small></span><select value={feedbackForm.gpu_asset_id} onChange={event => setFeedbackForm(current => ({ ...current, gpu_asset_id: event.target.value }))}><option value="">{tx('选择发生故障的 GPU', 'Select failed GPU')}</option>{activeAssets.map(asset => <option key={asset.id} value={asset.id}>{asset.node_ip} · GPU {asset.gpu_index} · {asset.gpu_uuid || 'UUID UNKNOWN'} · {asset.model_name || asset.model || 'MODEL UNKNOWN'}</option>)}</select></label>
          ...
          <label><span>{tx('维修/更换结论', 'Repair/replacement result')}<small>OPTIONAL · DISPOSITION</small></span><select value={feedbackForm.repair_action} onChange={event => setFeedbackForm(current => ({ ...current, repair_action: event.target.value }))}><option value="pending">{tx('待确认', 'Pending')}</option><option value="reseat">{tx('重新插拔 / 复位', 'Reseat / reset')}</option><option value="replace_gpu">{tx('更换 GPU', 'Replace GPU')}</option><option value="replace_node">{tx('整机维修 / 更换', 'Node repair / replacement')}</option><option value="no_hardware_fault">{tx('排除硬件故障', 'No hardware fault')}</option></select></label>
          ...
          {selectedFeedbackAsset ? <p className="node-access-note">{tx('将请求离线获取：', 'Will request offline collection:')} {selectedFeedbackAsset.node_ip} · GPU {selectedFeedbackAsset.gpu_index} · {selectedFeedbackAsset.gpu_uuid || 'UUID UNKNOWN'} · T-{feedbackForm.pre_window_hours}h / T+{feedbackForm.post_window_hours}h</p> : null}
