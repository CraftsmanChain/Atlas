...
type ValidationReadinessReport = { version: string; framework_version: string; mode: string; status: string; readiness_sha256: string; label_gate_status: string; label_manifest_version: string; label_manifest_sha256: string; outcome_report_version: string; outcome_stability_status: string; outcome_maturity: { total: number; matured: number; pending: number; censored: number; matured_ratio: number; probability_scored: number; node_eligible: number }; challenger_version: string; challenger_status: string; challenger_confidence_status: string; seven_day_rows: number; seven_day_nodes: number; seven_day_positives: number; seven_day: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; ranking_at_k?: RankingAtK[] }>; blocking_reasons: string[]; recommended_next_run: string[]; generated_at: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="prediction-contract">
          <span><b>{tx('绑定报告', 'Bound reports')}</b>{validationReadiness.outcome_report_version}<small>{validationReadiness.challenger_version}</small></span>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.12.0',
    status: tx('训练样本证据包', 'TRAINING EVIDENCE BUNDLE'),
    desc: tx(
