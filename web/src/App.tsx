...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.4.1', status: tx('双源质量基线', 'DUAL-SOURCE BASELINE'), desc: tx('识别 Target 异常、指标缺失或陈旧、覆盖不足、身份冲突和采集链路问题；双源数值差异仅保留为取值审计，不定性为数据异常。', 'Detect target failures, missing or stale metrics, coverage gaps, identity conflicts and collection-path issues; cross-source value differences remain audit evidence rather than data anomalies.'), history: [tx('v0.4.1 · 双源数值差异改为审计信息，不再生成数据质量问题', 'v0.4.1 · cross-source value differences became audit-only and no longer create data-quality issues'), tx('v0.4.0 · DCGM 与 gpu_exporter 规范化一致性观察', 'v0.4.0 · normalized DCGM/gpu_exporter consistency observations'), tx('v0.3.0 · 统一数据时效 API、SLA 边界和 8077 源时间标识', 'v0.3.0 · unified freshness API, SLA boundaries and source-time indicator on 8077'), tx('v0.2.0 · 原因分类、节点抑制与同步审计', 'v0.2.0 · reason classification, node suppression and sync audit')] },
    ...
    { id: 'issue', name: tx('问题统计与处置', 'Issue Analytics & Resolution'), version: 'v0.2.1', status: tx('问题闭环基线', 'ISSUE WORKFLOW BASELINE'), desc: tx('统一统计平台确认的问题、分类、自动检测状态和人工处置状态，记录根因、方案、处理过程与结果，并输出可审计的 AI/Skill 训练数据；双源数值差异不进入问题台账。', 'Normalize confirmed platform issues, categories, automated detection and human workflow states; capture resolution evidence for audited AI/Skill datasets; cross-source value differences do not enter the issue ledger.'), history: [tx('v0.2.1 · 停止生成双源数值差异问题，历史记录退出列表、统计和训练数据', 'v0.2.1 · stopped creating source-value-difference issues; legacy records are excluded from lists, analytics and training data'), tx('v0.2.0 · 双源一致性问题试运行', 'v0.2.0 · source-consistency issue trial'), tx('v0.1.0 · 五类检测源归一化、统计钻取、处置历史与训练数据导出', 'v0.1.0 · five-source normalization, analytics drill-down, resolution history and training export')] },
