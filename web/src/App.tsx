...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  const buildManualFeedbackFeatureRequestManifest = async () => {
    ...
    try {
      ...
      const buildsResponse = await fetch('/api/v1/prediction/history/feedback-feature-requests');
      if (!buildsResponse.ok) throw new Error(`HTTP ${buildsResponse.status}`);
      const payload = await buildsResponse.json();
      ...
      setFeedbackFeatureRequestNotice(created ? tx(
        `特征请求 Manifest 已生成：${String(created.status || '').toUpperCase()} · ${created.window_count || 0} 窗口 · ${created.manifest_sha256 || created.blocking_reasons?.[0] || '等待 SHA'}`,
        `Feature request manifest created: ${String(created.status || '').toUpperCase()} · ${created.window_count || 0} windows · ${created.manifest_sha256 || created.blocking_reasons?.[0] || 'waiting for SHA'}`,
  ...
  const runManualFeedbackFeatureRequestWorker = async () => {
    ...
    try {
      ...
      setFeedbackFeatureRequestNotice(current ? tx(
        `离线特征聚合已启动/更新：${String(current.status || '').toUpperCase()} · ${current.completed_windows || 0}/${current.failed_windows || 0} 窗口 · ${current.feature_sha256 || current.error_message || '等待特征 SHA'}`,
        `Offline feature aggregation started/updated: ${String(current.status || '').toUpperCase()} · ${current.completed_windows || 0}/${current.failed_windows || 0} windows · ${current.feature_sha256 || current.error_message || 'waiting for feature SHA'}`,
      ) : '');
      setPredictionError('');
    } catch (reason) {
      setFeedbackFeatureRequestNotice('');
    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'feedback-pack' ? <Card id="prediction-feedback-package" className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('最新特征请求', 'Latest feature request')}</b><strong>{latestFeedbackFeatureRequest?.status?.toUpperCase() || 'NOT STARTED'}</strong><small>{latestFeedbackFeatureRequest?.version || 'manual-feedback-feature-request-v1'}</small></div>
          ...
          <div><b>{tx('Manifest 指纹', 'Manifest SHA')}</b><strong>{latestFeedbackFeatureRequest?.manifest_sha256?.slice(0, 16) || '—'}</strong><small>{latestFeedbackFeatureRequest?.blocking_reasons?.[0] || latestFeedbackFeatureRequest?.error_message || tx('生成后这里会立即更新', 'Updates here immediately after creation')}</small></div>
          <div><b>{tx('离线聚合', 'Offline aggregation')}</b><strong>{latestFeedbackFeatureRequest ? `${latestFeedbackFeatureRequest.completed_windows || 0}/${latestFeedbackFeatureRequest.failed_windows || 0}` : '—'}</strong><small>{latestFeedbackFeatureRequest?.feature_sha256?.slice(0, 16) || tx('Manifest ready 后可运行', 'Run after Manifest is ready')}</small></div>
        </div>
        <div className="table-actions"><button className="link" type="button" disabled={feedbackFeatureRequestRunning || latestFeedbackFeatureRequest?.status !== 'manifest_ready_pending_offline_worker'} onClick={() => void runManualFeedbackFeatureRequestWorker()}>{feedbackFeatureRequestRunning ? tx('聚合中…', 'Aggregating…') : tx('运行离线特征聚合', 'Run offline aggregation')}</button><span className="muted">{tx('如果 Manifest 被阻断，先处理列表里的准备/复核/身份解析状态。', 'If the Manifest is blocked, fix preparation/review/identity status in the list first.')}</span></div>
        <div className="table-wrap"><table><thead><tr><th>{tx('故障对象', 'Fault target')}</th><th>{tx('故障类型 / 时间', 'Fault type / time')}</th><th>{tx('窗口', 'Window')}</th><th>{tx('采集状态', 'Collection status')}</th><th>{tx('身份解析', 'Identity resolution')}</th><th>{tx('预警复核', 'Warning review')}</th><th>{tx('维修结论', 'Repair result')}</th><th>{tx('反馈人', 'Operator')}</th><th>{tx('动作', 'Action')}</th></tr></thead><tbody>{faultFeedbackRequests.map(row => <tr key={row.id}><td><b>{row.node_ip} · {(row.target_scope || 'gpu').toUpperCase()}{(row.target_scope || 'gpu') === 'gpu' ? ` · GPU ${row.gpu_index}` : ''}</b><small>{(row.affected_gpu_indexes || []).length ? `GPU ${(row.affected_gpu_indexes || []).join(',')}` : (row.gpu_uuid || tx('节点/底板级，无需单卡 UUID', 'Node/baseboard scope; no single GPU UUID required'))} · {row.reported_gpu_uuid ? `${tx('当前/上报', 'reported')} ${row.reported_gpu_uuid}` : (row.model_name || 'MODEL UNKNOWN')}</small></td><td><b>{row.fault_type}</b><small>{(row.fault_time_precision || 'exact').toUpperCase()} · {row.fault_time_precision === 'window' && row.fault_window_start_at && row.fault_window_end_at ? `${time(row.fault_window_start_at, lang)} ~ ${time(row.fault_window_end_at, lang)}` : time(row.fault_occurred_at, lang)}</small></td><td><strong>T-{row.pre_window_hours}h / T+{row.post_window_hours}h</strong><small>{row.history_pack_scope}</small></td><td><Badge value={row.history_pack_status.toUpperCase()} kind={row.history_pack_status.includes('blocked') ? 'warning' : row.history_pack_status.includes('ready') ? 'healthy' : row.history_pack_status.includes('queued') ? 'warning' : 'info'} /><small>{(row.blocking_reasons || []).join(' · ') || row.history_pack_sha256 || row.status}</small></td><td><Badge value={(row.identity_resolution_status || 'current_identity_selected').toUpperCase()} kind={row.identity_resolution_status === 'requires_historical_identity_at_fault_time' || row.identity_resolution_status?.startsWith('blocked') ? 'warning' : 'healthy'} /><small>{row.identity_resolution_note || tx('当前身份可用于故障窗口', 'Current identity usable for fault window')}</small></td><td><Badge value={(row.warning_review_status || 'pending_warning_review').toUpperCase()} kind={row.warning_review_status?.includes('no_prior') || row.warning_review_status?.includes('below_threshold') || row.warning_review_status?.startsWith('blocked') ? 'warning' : row.warning_review_status?.includes('candidate_found') ? 'healthy' : 'info'} /><small>{row.matched_warning_count ?? 0} {tx('条 shadow 记录', 'shadow records')} · {row.warning_review_window_hours || row.pre_window_hours}h</small><small>{row.warning_review_note || tx('等待复核故障前影子预警覆盖', 'Waiting to review prior shadow-warning coverage')}</small></td><td><b>{row.repair_action || '—'}</b><small>{row.hardware_replaced ? tx('硬件已更换/维修', 'Hardware repaired/replaced') : tx('未确认更换硬件', 'No replacement confirmed')}</small></td><td><b>{row.operator}</b><small>{time(row.created_at, lang)} · {row.training_eligible ? 'TRAINING' : 'NO TRAINING'}</small></td><td><button className="link" type="button" disabled={preparingFeedbackID === row.id || row.history_pack_status === 'manifest_ready_pending_metric_extraction'} onClick={() => void prepareFeedbackPack(row.id)}>{preparingFeedbackID === row.id ? tx('准备中…', 'Preparing…') : row.history_pack_status === 'manifest_ready_pending_metric_extraction' ? tx('已准备', 'Ready') : tx('准备数据包', 'Prepare pack')}</button><button className="link" type="button" disabled={reviewingFeedbackID === row.id} onClick={() => void reviewFeedbackWarning(row.id)}>{reviewingFeedbackID === row.id ? tx('复核中…', 'Reviewing…') : tx('复核预警', 'Review warning')}</button><small>{(row.matched_warning_keys || [])[0] || (row.history_pack_sha256 ? row.history_pack_sha256.slice(0, 16) : row.status)}</small></td></tr>)}</tbody></table>{faultFeedbackRequests.length === 0 && <Empty tx={tx} title={tx('还没有人工反馈产生的数据包请求', 'No data-pack requests from human feedback yet')} />}</div>
      </Card> : null}
      ...
      <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('请求状态', 'Request status')}</b><strong>{latestFeedbackFeatureRequest?.status?.toUpperCase() || 'NOT STARTED'}</strong><small>{latestFeedbackFeatureRequest?.version || 'manual-feedback-feature-request-v1'}</small></div>
