...
type TrainingFeatureBuild = { id: number; feature_dataset_key: string; version: string; status: string; source_key: string; source_dataset_build_id: number; source_dataset_key: string; prediction_target: 'gpu_hardware_failure' | 'high_priority_xid_event'; feature_contract_version: string; lookback_minutes: number; query_step_seconds: number; episode_count: number; window_count: number; processed_episodes: number; completed_windows: number; failed_windows: number; metric_count: number; feature_column_count: number; average_metric_coverage: number; minimum_metric_coverage: number; output_dir: string; feature_path?: string; feature_sha256?: string; quality_report_path?: string; error_message?: string; started_at: string; finished_at?: string };
  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        <p className="node-access-note">{tx('按 GPU episode 批量读取 Prometheus/VM：所有预测窗保留 24h@5m 细粒度统计，仅 7 天目标额外计算 3d/7d/30d@1h 长周期趋势。长周期覆盖和健康对照污染单独门禁；所有样本时间必须早于 feature_cutoff_at，原始多年时序不会复制到开发机。', 'Batch Prometheus/VM metrics by GPU episode: every horizon retains fine-grained 24h@5m statistics, while only the 7-day target adds 3d/7d/30d@1h trends. Long-range coverage and healthy-control contamination are gated independently; every sample must precede feature_cutoff_at and multi-year raw series never leave the deployment node.')}</p>
        <div className="issue-categories quality-ledger">
          <div><b>{tx('任务状态', 'Build status')}</b><strong>{latestFeatureBuild?.status?.toUpperCase() || 'NOT STARTED'}</strong><small>{latestFeatureBuild?.version || 'gpu-historical-features-v5'}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [...baseModules, releaseModule].map(module => module.id === 'feature' ? {
    ...module,
    version: 'v1.11.0',
    status: tx('核心与可选信号分离', 'CORE / OPTIONAL SIGNALS'),
    desc: tx('历史预测扩展到 28 个候选指标族；核心遥测决定覆盖率，稀疏 PCIe、ECC、Row Remap、NVLink、限制计数和 metric-family 信号缺失保持 unknown。', 'Historical prediction now covers 28 candidate metric families; core telemetry determines coverage while missing sparse PCIe, ECC, row-remap, NVLink, violation-counter, and metric-family signals remain unknown.'),
