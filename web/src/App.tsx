    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('切片就绪门', 'Slice readiness')}</b><strong>{validationReadiness.slice_audit_status?.toUpperCase() || 'UNKNOWN'}</strong><small>{validationReadiness.slice_ready_dimensions}/4 READY · {validationReadiness.slice_partial_dimensions} PARTIAL · {validationReadiness.validation_slice_count} SLICES · SHA {validationReadiness.validation_slices_sha256?.slice(0, 10) || '—'} · RANK {validationReadiness.comparable_ranking_slices} / PROB {validationReadiness.comparable_probability_slices}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.25.2',
    status: tx('切片就绪门', 'SLICE READINESS GATE'),
    desc: tx(
