    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'feedback-pack' ? <Card id="prediction-feedback-package" className="span-12">
        ...
        <div className="issue-categories quality-ledger compact-ledger">
          <div><b>{tx('最新特征请求', 'Latest feature request')}</b><strong>{compactWorkflowStatus(latestFeatureStatus)}</strong><small>{latestFeedbackFeatureRequest?.version || 'manual-feedback-feature-request-v5'}</small></div>
      ...
      <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('任务状态', 'Build status')}</b><strong>{latestFeatureBuild?.status?.toUpperCase() || 'NOT STARTED'}</strong><small>{latestFeatureBuild?.version || 'gpu-historical-features-v4'}</small></div>
      ...
      <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('请求状态', 'Request status')}</b><strong>{compactWorkflowStatus(latestFeatureStatus)}</strong><small>{latestFeedbackFeatureRequest?.version || 'manual-feedback-feature-request-v1'}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [...baseModules, releaseModule].map(module => module.id === 'feature' ? {
    ...
    history: [tx('v1.10.0 · 拆分核心覆盖与可选信号，扩展线上已核验的历史预测指标', 'v1.10.0 · separates core coverage from optional signals and expands production-verified historical prediction metrics'), ...module.history],
