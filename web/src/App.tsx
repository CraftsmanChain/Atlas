  ...
  if (view === 'node-access') {
    ...
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.4.1'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('低负载只读信息按注册命令和固定资源预算自动采集，无需逐节点制定计划或人工确认；节点身份、GPU 快照和故障时间窗内核日志进入结构化证据包。', 'Low-impact read-only evidence is collected automatically through registered commands and fixed resource budgets without per-node planning or confirmation. Node identity, GPU snapshots, and bounded kernel logs feed the structured evidence bundle.')}</p></div></div>
