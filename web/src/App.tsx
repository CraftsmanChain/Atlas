  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        <p className="node-access-note">{tx('展示已持久化的训练分布与 live shadow 分布压缩快照；训练分布可从已完成 baseline 绑定的训练矩阵生成，live-shadow 分布由后续影子评分批次在内存聚合生成，只保存统计量和 histogram proportions，不保存原始遥测样本。Feature Drift Report 只有在同一 baseline 下训练与线上分布都齐全时才会计算逐特征 PSI/KS。', 'Shows persisted compact training and live-shadow feature distribution snapshots. Training distributions can be materialized from the completed baseline-bound training matrix; live-shadow distributions are aggregated in memory by subsequent shadow scoring batches. Only aggregate statistics and histogram proportions are stored, never raw telemetry samples. Feature Drift Report computes per-feature PSI/KS only when both training and live distributions exist for the same baseline.')}</p>
        <div className="issue-categories quality-ledger">
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.4',
    status: tx('验证就绪绑定特征漂移建议', 'READINESS FEATURE DRIFT GUIDANCE'),
    desc: tx(
