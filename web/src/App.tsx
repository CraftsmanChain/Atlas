  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="prediction-contract">
          <span><b>{tx('绑定报告', 'Bound reports')}</b>{validationReadiness.outcome_report_version}<small>{validationReadiness.challenger_version} · {validationReadiness.risk_ranking_version} · {validationReadiness.temporal_consistency_version} · {validationReadiness.slice_audit_version} · {validationReadiness.evidence_bundle_version} · {validationReadiness.data_drift_version} · {validationReadiness.calibration_drift_version} · {validationReadiness.feature_drift_version} · {validationReadiness.feature_distribution_archive_version}</small></span>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [...baseModules, releaseModule].map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.27.1',
    status: tx('人工反馈闭环清单', 'HUMAN FEEDBACK MANIFEST'),
    desc: tx(
