  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {featureDriftReport ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('特征列 / 分布', 'Feature columns / distributions')}</b><strong>{featureDriftReport.feature_column_count} / {featureDriftReport.feature_distribution_count}</strong><small>{featureDriftReport.feature_contract_version || 'NO CONTRACT'}</small></div>
          <div><b>PSI / KS</b><strong>{featureDriftReport.psi_status.toUpperCase()} / {featureDriftReport.ks_status.toUpperCase()}</strong><small>{featureDriftReport.blocking_reasons[0] || featureDriftReport.recommended_next_run[0]}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.16.0',
    status: tx('特征漂移就绪门', 'FEATURE DRIFT READINESS GATE'),
    desc: tx(
      '当前预测能力保持 GPU-only、read-only shadow：已具备训练数据、标签 Manifest、训练样本 Evidence Bundle、模型治理卡、影子门禁、成熟 outcome、Ranking@K、naive baseline、HeaRank 7d node-risk challenger、数据/校准漂移和特征漂移就绪门，但不触发告警、调度、维修或自动隔离。',
      'Prediction remains GPU-only and read-only shadow: training data, label manifest, training-sample Evidence Bundle, model governance cards, shadow gates, mature outcomes, Ranking@K, naive baselines, a HeaRank 7d node-risk challenger, data/calibration drift, and feature-drift readiness gates are available, but no alert, scheduling, repair or automatic isolation is triggered.',
    ),
    history: [
