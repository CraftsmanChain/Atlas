...
type PredictionOutcomeReport = { version: string; framework_version: string; mode: string; safety: { read_only_shadow: boolean; no_alert_emitted: boolean; no_action_taken: boolean; probability_use: string }; sample_maturity: { total: number; matured: number; pending: number; censored: number; matured_ratio: number; probability_scored: number; node_eligible: number }; accuracy: PredictionAccuracy; interpretation: string[]; recommended_next_run: string[]; generated_at: string };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, accuracyResponse, outcomeReportResponse, governanceResponse, outcomesResponse, resultsResponse, parityResponse, replayResponse, coverageResponse, shadowResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/accuracy'), fetch('/api/v1/prediction/outcome-report'), fetch('/api/v1/prediction/model-governance'), fetch('/api/v1/prediction/outcomes'), fetch('/api/v1/prediction/results'), fetch('/api/v1/prediction/feature-parity'), fetch('/api/v1/prediction/history/feature-replays'), fetch('/api/v1/prediction/history/live-coverage'), fetch('/api/v1/prediction/history/shadow-scoring'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !accuracyResponse.ok || !outcomeReportResponse.ok || !governanceResponse.ok || !outcomesResponse.ok || !resultsResponse.ok || !parityResponse.ok || !replayResponse.ok || !coverageResponse.ok || !shadowResponse.ok || !historyResponse.ok || !backfillResponse.ok || !candidatesResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${accuracyResponse.status}/${outcomeReportResponse.status}/${governanceResponse.status}/${outcomesResponse.status}/${resultsResponse.status}/${parityResponse.status}/${replayResponse.status}/${coverageResponse.status}/${shadowResponse.status}/${historyResponse.status}/${backfillResponse.status}/${candidatesResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, accuracyPayload, outcomeReportPayload, governancePayload, outcomesPayload, resultsPayload, parityPayload, replayPayload, coveragePayload, shadowPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), accuracyResponse.json(), outcomeReportResponse.json(), governanceResponse.json(), outcomesResponse.json(), resultsResponse.json(), parityResponse.json(), replayResponse.json(), coverageResponse.json(), shadowResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
