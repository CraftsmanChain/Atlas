...
type PredictionShadowScoringRun = { id: number; run_key: string; version: string; status: string; trigger: string; model_spec_id: number; model_key: string; model_version: string; artifact_sha256: string; source_key: string; scope_model_name: string; transformation_contract_version: string; window_minutes: number; query_step_seconds: number; target_gpu_count: number; scored_gpu_count: number; blocked_gpu_count: number; positive_gpu_count: number; minimum_probability?: number; maximum_probability?: number; mean_probability?: number; blocking_reasons: string[]; no_alert_emitted: boolean; no_action_executed: boolean; report_sha256?: string; error_message?: string; started_at: string; finished_at?: string };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, accuracyResponse, outcomesResponse, resultsResponse, parityResponse, replayResponse, coverageResponse, shadowResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/accuracy'), fetch('/api/v1/prediction/outcomes'), fetch('/api/v1/prediction/results'), fetch('/api/v1/prediction/feature-parity'), fetch('/api/v1/prediction/history/feature-replays'), fetch('/api/v1/prediction/history/live-coverage'), fetch('/api/v1/prediction/history/shadow-scoring'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !accuracyResponse.ok || !outcomesResponse.ok || !resultsResponse.ok || !parityResponse.ok || !replayResponse.ok || !coverageResponse.ok || !shadowResponse.ok || !historyResponse.ok || !backfillResponse.ok || !candidatesResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${accuracyResponse.status}/${outcomesResponse.status}/${resultsResponse.status}/${parityResponse.status}/${replayResponse.status}/${coverageResponse.status}/${shadowResponse.status}/${historyResponse.status}/${backfillResponse.status}/${candidatesResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, accuracyPayload, outcomesPayload, resultsPayload, parityPayload, replayPayload, coveragePayload, shadowPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), accuracyResponse.json(), outcomesResponse.json(), resultsResponse.json(), parityResponse.json(), replayResponse.json(), coverageResponse.json(), shadowResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {shadowModelCandidates.length > 0 ? <Card className="span-12">
        ...
        <p className="node-access-note">{tx('人工触发的只读前瞻评分：每批重新验证模型 SHA256、当前 24 小时数据覆盖和 70 列特征契约，通过后保存概率与特征指纹。概率仅用于影子验证，不发告警、不改健康分、不执行隔离或重启。', 'Manually triggered read-only prospective scoring. Every run re-verifies the model SHA256, current trailing-24h coverage, and all 70 feature columns before persisting probability and a feature-vector fingerprint. Probabilities are shadow evidence only: no alert, health-score change, isolation, or restart.')}</p>
        {latestShadowScoring ? <div className="issue-categories quality-ledger"><div><b>{tx('批次状态', 'Run status')}</b><strong>{latestShadowScoring.status.toUpperCase()}</strong><small>{latestShadowScoring.version}</small></div><div><b>{tx('已评分 / 目标 GPU', 'Scored / target GPUs')}</b><strong>{latestShadowScoring.scored_gpu_count} / {latestShadowScoring.target_gpu_count}</strong><small>{latestShadowScoring.blocked_gpu_count} BLOCKED</small></div><div><b>{tx('超阈值 GPU', 'Above-threshold GPUs')}</b><strong>{latestShadowScoring.positive_gpu_count}</strong><small>THRESHOLD {shadowModelCandidates[0]?.decision_threshold?.toFixed(3) || '—'}</small></div><div><b>{tx('概率范围 / 均值', 'Probability range / mean')}</b><strong>{latestShadowScoring.minimum_probability === undefined ? '—' : `${(latestShadowScoring.minimum_probability * 100).toFixed(2)}%–${((latestShadowScoring.maximum_probability || 0) * 100).toFixed(2)}%`}</strong><small>MEAN {latestShadowScoring.mean_probability === undefined ? '—' : `${(latestShadowScoring.mean_probability * 100).toFixed(2)}%`} · {latestShadowScoring.error_message || latestShadowScoring.report_sha256?.slice(0, 16) || 'RUNNING'}</small></div></div> : <Empty tx={tx} title={tx('覆盖门通过，等待首次人工触发影子评分', 'Coverage gate passed; waiting for the first manual shadow run')} />}
        {highestRiskPredictions.length > 0 ? <div className="table-wrap"><table><thead><tr><th>{tx('节点 / GPU', 'Node / GPU')}</th><th>{tx('影子概率', 'Shadow probability')}</th><th>{tx('风险级别', 'Risk level')}</th><th>{tx('模型 / 窗口', 'Model / horizon')}</th><th>{tx('特征指纹', 'Feature fingerprint')}</th></tr></thead><tbody>{highestRiskPredictions.map(row => <tr key={row.id}><td><b>{row.node_ip}</b><small>{row.gpu_uuid}</small></td><td><strong>{((row.probability || 0) * 100).toFixed(3)}%</strong></td><td><Badge value={row.risk_level.toUpperCase()} kind={row.risk_level === 'high' ? 'warning' : 'info'} /><small>{row.status.toUpperCase()}</small></td><td><b>{row.model_version}</b><small>{row.horizon_minutes}m</small></td><td><code>{row.feature_vector_sha256?.slice(0, 16) || '—'}</code><small>{row.transformation_contract_version}</small></td></tr>)}</tbody></table></div> : null}
      </Card> : null}
      <Card className="span-12">
