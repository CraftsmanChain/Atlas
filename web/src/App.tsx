  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="prediction-contract">
          <span><b>{tx('绑定报告', 'Bound reports')}</b>{validationReadiness.outcome_report_version}<small>{validationReadiness.challenger_version} · {validationReadiness.evidence_bundle_version} · {validationReadiness.data_drift_version} · {validationReadiness.calibration_drift_version} · {validationReadiness.feature_drift_version}</small></span>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.6',
    status: tx('验证范围分布归档', 'SCOPED DISTRIBUTION ARCHIVE'),
    desc: tx(
