  ...
  if (view === 'node-access') {
    ...
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.5.0'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('低负载只读信息按注册命令和固定资源预算自动采集；节点失联时等待恢复，恢复为 up 后补采启动历史和故障时间窗证据。', 'Low-impact read-only evidence is collected through registered commands and fixed budgets; offline nodes wait until recovery, then collect boot history and incident-window evidence after returning up.')}</p></div></div>
