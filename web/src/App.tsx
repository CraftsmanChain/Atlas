  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          <div><b>{tx('任务状态', 'Build status')}</b><strong>{latestFeatureBuild?.status?.toUpperCase() || 'NOT STARTED'}</strong><small>{latestFeatureBuild?.version || 'gpu-historical-features-v3'}</small></div>
      ...
      <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('平均指标覆盖率', 'Average metric coverage')}</b><strong>{latestFeedbackFeatureRequest?.feature_sha256 ? `${((latestFeedbackFeatureRequest.average_metric_coverage || 0) * 100).toFixed(1)}%` : '—'}</strong><small>MIN {latestFeedbackFeatureRequest?.feature_sha256 ? `${((latestFeedbackFeatureRequest.minimum_metric_coverage || 0) * 100).toFixed(1)}%` : '—'} · {latestFeedbackFeatureRequest?.feature_column_count || 0} COLUMNS</small></div>
