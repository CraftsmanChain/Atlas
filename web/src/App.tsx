  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      {observabilityRanking ? <Card className="span-12">
        <CardHead code={observabilityRanking.version} title={tx('结构可观测性前瞻排序验证', 'Prospective Observability Ranking Validation')} action={<div className="table-actions"><a className="link" href="/api/v1/prediction/observability-ranking-validation?download=1" download><Download size={13} />{tx('下载验证报告', 'Download validation')}</a><Badge value={observabilityRanking.status.toUpperCase()} kind={observabilityRanking.status === 'comparable' ? 'healthy' : observabilityRanking.status === 'exploratory' ? 'info' : 'warning'} /><Badge value="MODEL-INDEPENDENT" kind="info" /></div>} />
        <p className="node-access-note">{tx('该报告完全绕开模型注册表：在相同独立 cohort、截点和未来标签上固定赛马 max-gap、存在率缺口、样本年龄、UUID 波动、抓取成功率缺口、抓取样本量缺口和连续性最差裕量七种结构策略。负例额外等待 24 小时删失期；同分策略不进入一致性判断，结果只表达相对复核优先级，不是故障概率。', 'This report bypasses the model registry and races seven fixed structural policies on identical independent cohorts, cutoffs, and future labels: max gap, presence deficit, sample age, UUID flaps, scrape-success deficit, scrape-sample deficit, and worst continuity margin. Negative outcomes wait through an additional 24-hour censoring period; constant-score policies do not enter consistency evaluation. Results are relative review priority, not failure probability.')}</p>
        <div className="issue-categories quality-ledger">
