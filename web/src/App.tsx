  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'feedback-pack' ? <Card id="prediction-feedback-package" className="span-12">
        <CardHead code="PRE/POST HISTORY PACKS" title={tx('故障前后数据反馈包', 'Pre/Post-fault Data Feedback Pack')} action={<div className="table-actions"><Badge value={`${faultFeedbackRequests.length} REQUESTS`} kind="info" /><Badge value="OFFLINE COLLECTION" kind="warning" /></div>} />
        <p className="node-access-note">{tx('这里显示人工反馈产生的前后监控历史数据包请求。当前状态只表示已排队等待离线抓取；后续 worker 会按节点、GPU、故障时间和窗口固化特征、标签和原始监控片段。', 'This shows pre/post monitoring-history pack requests created from human feedback. The current status means queued for offline collection; a later worker will materialize features, labels, and raw telemetry slices by node, GPU, fault time, and window.')}</p>
