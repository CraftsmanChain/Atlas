...
type DataDriftReport = { version: string; framework_version: string; mode: string; status: string; report_sha256: string; method: string; minimum_shadow_runs: number; psi_proxy_threshold: number; ks_proxy_threshold: number; positive_ratio_delta_threshold: number; latest?: ShadowDistributionSnapshot; baseline?: ShadowDistributionSnapshot; psi_proxy: number; ks_proxy: number; positive_ratio_delta: number; median_delta: number; p95_delta: number; p99_delta: number; blocking_reasons: string[]; recommended_next_run: string[]; generated_at: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {dataDriftReport ? <Card className="span-12">
        ...
        <p className="node-access-note">{tx('基于最近两次只读 shadow scoring 的持久化概率分布统计计算 PSI/KS proxy。当前这是影子分数分布漂移摘要，不替代后续逐特征 PSI/KS；不会调阈值、不会触发告警或动作。', 'Computes PSI/KS proxies from persisted probability-distribution statistics of the two latest read-only shadow scoring runs. This is a shadow-score distribution drift summary, not a replacement for future per-feature PSI/KS; it changes no threshold and triggers no alert or action.')}</p>
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('漂移门', 'Drift gate')}</b><strong>{validationReadiness.data_drift_status.toUpperCase()}</strong><small>PSI {validationReadiness.data_drift_psi_proxy.toFixed(3)} · KS {validationReadiness.data_drift_ks_proxy.toFixed(3)}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.13.0',
    status: tx('影子分布漂移门', 'SHADOW DISTRIBUTION DRIFT GATE'),
    desc: tx(
