...
type ModelRaceComparison = { version: string; reference_build_id: number; build_ids: number[]; source_matrix_build_id: number; source_training_matrix_key: string; matrix_sha256: string; comparison_sha256: string; builds: Array<{ build_id: number; model_key: string; version: string; algorithm: string; macro_test: BaselineMetrics; stable_count: number; candidate_count: number; selected_source_metric_count: number; selected_source_metrics: string[] }>; deltas_vs_reference: Array<{ build_id: number; reference_build_id: number; roc_auc: number; pr_auc: number; precision: number; recall: number }>; horizons: Array<{ horizon_minutes: number; selected_feature_count_by_build_id: Record<string, number>; selected_source_metrics_by_build_id: Record<string, string[]> }> };
...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  useEffect(() => {
    ...
    const reference = sameMatrix.find(build => build.algorithm === 'logistic_regression');
    const challengers = sameMatrix.filter(build => build.id !== reference?.id && !sameMatrix.some(earlier => earlier.id > build.id && earlier.algorithm === build.algorithm));
  ...
  const trainBaselineModels = async (algorithm: 'logistic_regression' | 'gradient_boosted_stumps' | 'anomaly_filtered_logistic' | 'anomaly_augmented_logistic' = 'logistic_regression') => {
    ...
    try {
      const response = await fetch('/api/v1/prediction/history/baseline-models', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source_matrix_build_id: trainingMatrixBuilds[0].id, event_type: readyScope.event_type, model_name: readyScope.model_name, algorithm }) });
