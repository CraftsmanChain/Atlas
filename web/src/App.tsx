  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      {observabilityRanking ? <Card className="span-12">
        ...
        <p className="node-access-note">{tx('该报告完全绕开模型注册表：每个独立 cohort 直接冻结一次健康评估批次的 1 小时 max-gap 节点分数，并用随后 7 天内 confirmed / strong-proxy 故障标签评估 Top-5% 捕获和 lift。负例额外等待 24 小时删失期；结果只表达相对复核优先级，不是故障概率。', 'This report bypasses the model registry entirely: each independent cohort freezes one health evaluation run and ranks nodes by the persisted one-hour max-gap signal, then evaluates Top-5% capture and lift against confirmed or strong-proxy labels in the following seven days. Negative outcomes wait through an additional 24-hour censoring period. Results are relative review priority, not failure probability.')}</p>
        <div className="issue-categories quality-ledger">
          <div><b>{tx('报告指纹', 'Report SHA')}</b><strong>{observabilityRanking.report_sha256.slice(0, 16)}</strong><small>{observabilityRanking.signal}</small></div>
