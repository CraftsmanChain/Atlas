...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {humanFeedbackManifest ? <Card className="span-12">
      ...
      {validationReadiness ? <Card className="span-12">
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [...baseModules, releaseModule].map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.27.2',
    status: tx('反馈证据绑定就绪门', 'FEEDBACK EVIDENCE BOUND'),
    desc: tx(
      '当前预测能力保持 GPU-only、read-only shadow：已具备训练数据、标签 Manifest、训练样本 Evidence Bundle、模型治理卡、影子门禁、成熟 outcome、Ranking@K、naive baseline、HeaRank 7d node-risk challenger、数据/校准漂移、训练侧和 live-shadow 特征分布快照，但不触发告警、调度、维修或自动隔离。',
      'Prediction remains GPU-only and read-only shadow: training data, label manifest, training-sample Evidence Bundle, model governance cards, shadow gates, mature outcomes, Ranking@K, naive baselines, a HeaRank 7d node-risk challenger, data/calibration drift, and training/live-shadow feature-distribution snapshots are available, but no alert, scheduling, repair or automatic isolation is triggered.',
    ),
    history: [
