...
type LabelManifest = { version: string; label_contract_version: string; mode: string; total: number; positive: number; excluded: number; confirmed: number; strong_proxy: number; weak_proxy: number; human_confirmed: number; by_event_type: Record<string, number>; by_model: Record<string, number>; by_source_type: Record<string, number>; by_rule_version: Record<string, number>; exclusion_reasons: Record<string, number>; positive_policy: string[]; negative_policy: string; gray_sample_policy: string[]; human_evidence_policy: string[]; known_gaps: string[]; generated_at: string };
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {labelManifest ? <Card className="span-12">
        <CardHead code={labelManifest.version} title={tx('标签 Manifest', 'Label Manifest')} action={<div className="table-actions"><Badge value={labelManifest.mode.toUpperCase()} kind="info" /><Badge value={`${labelManifest.total} LABELS`} kind="healthy" /></div>} />
        ...
        <div className="prediction-contract">
          ...
          <span><b>{tx('更新时间', 'Generated')}</b>{time(labelManifest.generated_at, lang)}<small>{Object.entries(labelManifest.by_event_type || {}).slice(0, 3).map(([key, value]) => `${key} ${value}`).join(' · ') || 'NO EVENT TYPES'}</small></span>
        </div>
      </Card> : null}
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.8.0',
    status: tx('标签 Manifest 只读治理', 'READ-ONLY LABEL MANIFEST'),
    desc: tx(
