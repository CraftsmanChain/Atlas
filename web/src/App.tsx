...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, accuracyResponse, outcomeReportResponse, governanceResponse, labelManifestResponse, evidenceBundleResponse, dataDriftResponse, challengerResponse, validationReadinessResponse, outcomesResponse, resultsResponse, parityResponse, replayResponse, coverageResponse, shadowResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/accuracy'), fetch('/api/v1/prediction/outcome-report'), fetch('/api/v1/prediction/model-governance'), fetch('/api/v1/prediction/label-manifest'), fetch('/api/v1/prediction/evidence-bundle'), fetch('/api/v1/prediction/data-drift-report'), fetch('/api/v1/prediction/hearank-challenger'), fetch('/api/v1/prediction/validation-readiness'), fetch('/api/v1/prediction/outcomes'), fetch('/api/v1/prediction/results'), fetch('/api/v1/prediction/feature-parity'), fetch('/api/v1/prediction/history/feature-replays'), fetch('/api/v1/prediction/history/live-coverage'), fetch('/api/v1/prediction/history/shadow-scoring'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !accuracyResponse.ok || !outcomeReportResponse.ok || !governanceResponse.ok || !labelManifestResponse.ok || !evidenceBundleResponse.ok || !dataDriftResponse.ok || !challengerResponse.ok || !validationReadinessResponse.ok || !outcomesResponse.ok || !resultsResponse.ok || !parityResponse.ok || !replayResponse.ok || !coverageResponse.ok || !shadowResponse.ok || !historyResponse.ok || !backfillResponse.ok || !candidatesResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${accuracyResponse.status}/${outcomeReportResponse.status}/${governanceResponse.status}/${labelManifestResponse.status}/${evidenceBundleResponse.status}/${dataDriftResponse.status}/${challengerResponse.status}/${validationReadinessResponse.status}/${outcomesResponse.status}/${resultsResponse.status}/${parityResponse.status}/${replayResponse.status}/${coverageResponse.status}/${shadowResponse.status}/${historyResponse.status}/${backfillResponse.status}/${candidatesResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, accuracyPayload, outcomeReportPayload, governancePayload, labelManifestPayload, evidenceBundlePayload, dataDriftPayload, challengerPayload, validationReadinessPayload, outcomesPayload, resultsPayload, parityPayload, replayPayload, coveragePayload, shadowPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), accuracyResponse.json(), outcomeReportResponse.json(), governanceResponse.json(), labelManifestResponse.json(), evidenceBundleResponse.json(), dataDriftResponse.json(), challengerResponse.json(), validationReadinessResponse.json(), outcomesResponse.json(), resultsResponse.json(), parityResponse.json(), replayResponse.json(), coverageResponse.json(), shadowResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="prediction-contract">
          <span><b>{tx('绑定报告', 'Bound reports')}</b>{validationReadiness.outcome_report_version}<small>{validationReadiness.challenger_version} · {validationReadiness.evidence_bundle_version} · {validationReadiness.data_drift_version}</small></span>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.14.0',
    status: tx('覆盖质量漂移门', 'COVERAGE QUALITY DRIFT GATE'),
    desc: tx(
