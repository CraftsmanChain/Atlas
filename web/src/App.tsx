...
function AssetStatistics({ tx, lang }: { tx: Tx; lang: string }) {
  ...
  const visible = rows.filter(row => (!scope || row.scope === scope) && (!category || row.category === category) && (!query.trim() || [row.ip_address, row.name, row.sn, row.type, row.gpu_model].join(' ').toLowerCase().includes(query.trim().toLowerCase())));
  ...
  return <div className="grid asset-statistics">
    ...
    <Card className="span-12 asset-difference-card">
      <CardHead code="SOURCE DIFFERENCE" title={tx('双接口差值', 'Cross-source Difference')} action={scope ? <button className="link" onClick={() => setScope('')}>{tx('取消筛选', 'Clear filter')}<X size={12} /></button> : undefined} />
      <div className="source-diff-grid">
        {[['both', tx('双边共有', 'In both'), tx('SN 或 IP 匹配', 'Matched by SN or IP'), CheckCircle2], ['ops_only', tx('仅运维主机接口', 'Ops host only'), tx('资产设备接口不存在', 'Missing from asset machines'), Server], ['asset_only', tx('仅资产设备接口', 'Asset machine only'), tx('运维主机接口不存在', 'Missing from ops hosts'), Database]].map(([key, label, note, Icon]) => { const I = Icon as typeof Server; const count = summary?.by_scope[String(key)] || 0; return <button key={String(key)} className={scope === key ? 'active' : ''} onClick={() => setScope(current => current === key ? '' : String(key))}><I size={19} /><span><b>{String(label)}</b><small>{String(note)}</small></span><strong>{count}</strong></button>; })}
      </div>
    </Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    { id: 'asset', name: tx('资产对账', 'Asset Reconciliation'), version: 'v0.3.1', status: tx('在线资产权威边界', 'LIVE-ASSET AUTHORITY'), desc: tx('以 LXOP 双接口实时在用资产为事实来源；仅 on / 已上架使用中进入 GPU 监控域，状态、IP 或接口成员变化在每轮 10 分钟单飞对账中生效。', 'Live in-use assets from both LXOP APIs are authoritative. Only on / 已上架使用中 assets enter the GPU monitoring domain; state, IP, and API membership changes apply in every single-flight 10-minute reconciliation.'), history: [tx('v0.3.1 · 节点列表默认只返回当前资产，退休节点仅通过显式 retired 审计筛选查询', 'v0.3.1 · node lists default to current assets; retired nodes require the explicit retired audit filter'), tx('v0.3.0 · 在线状态白名单成为统一监控边界，10 分钟内退休非在用或已移除节点并失效当前评分', 'v0.3.0 · in-use state allowlist became the monitoring boundary; non-in-use or removed nodes retire and current scores invalidate within 10 minutes'), tx('v0.2.1 · GPU 身份与 Target 合并为 10 分钟单飞对账，恢复状态同批更新', 'v0.2.1 · GPU identity and targets use one 10-minute single-flight reconciliation with same-run recovery'), tx('v0.2.0 · 明确 LXOPEasier 资产事实源；接口同步待接入', 'v0.2.0 · LXOPEasier established as source of truth; API sync pending'), tx('v0.1.2 · Target、身份增量与每日全量对账', 'v0.1.2 · target/identity incremental and daily full reconciliation'), tx('v0.1.1 · GPU 主机范围过滤与历史 UUID 恢复', 'v0.1.1 · GPU host filtering and historical UUID recovery')] },
    ...
    { id: 'issue', name: tx('数据统计', 'Data Statistics'), version: 'v0.6.1', status: tx('仅统计实时在用资产', 'LIVE ASSETS ONLY'), desc: tx('统一承载问题台账与实时在用资产统计；非在线或已从 LXOP 移除的节点不进入异常统计，并清理其自动监控误报历史。', 'Combines the issue ledger with live in-use asset statistics. Non-online or LXOP-removed nodes are excluded from anomaly counts and their automated monitoring false-positive history is removed.'), history: [tx('v0.6.1 · 资产分类和双边差值仅统计在用状态，退休节点自动监控误报从历史台账清理', 'v0.6.1 · asset categories and cross-source differences count in-use states only; retired-node automated false positives are purged from history'), tx('v0.6.0 · 新增资产统计子页、type 分类、GPU 型号分布、SN/IP 双接口对账与差值明细', 'v0.6.0 · asset statistics subpage, type classification, GPU model distribution, SN/IP cross-source reconciliation, and difference details'), tx('v0.5.1 · 问题分类补充节点访问分类和协调尺寸的分类图标', 'v0.5.1 · added the node-access category and consistently sized category icons'), tx('v0.5.0 · atlas-issue-training-v1 增加稳定快照、默认脱敏、质量门、训练/评估划分和页面导出', 'v0.5.0 · atlas-issue-training-v1 added stable snapshots, default redaction, quality gates, train/evaluation splits and UI export'), tx('v0.4.0 · 新增按分类的解决、遗留和当前检测统计', 'v0.4.0 · resolved, remaining and active counts by category'), tx('v0.3.0 · 更名为数据统计，硬件故障迁入告警中心且保留底层处置台账', 'v0.3.0 · renamed Data Statistics; hardware faults moved to Alert Center with resolution records retained')] },
