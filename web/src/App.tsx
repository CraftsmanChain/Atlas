...
type HistoryBackfillRun = { id: number; source_key: string; job_type: string; status: string; query_version: string; range_start: string; range_end: string; step_seconds: number; chunk_hours: number; chunks_total: number; chunks_completed: number; series_scanned: number; signal_points: number; candidates_created: number; candidates_updated: number; error_message?: string; started_at: string; finished_at?: string };
type HistoricalFaultCandidate = { id: number; candidate_key: string; source_key: string; gpu_uuid: string; node_ip: string; hostname: string; model_name: string; pci_bus_id: string; event_type: string; event_code: string; event_message: string; severity: string; quality_tier: string; review_status: string; source_alert_name: string; signal_samples: number; onset_at: string; detection_window_end_at: string };
type HistoricalCandidateSummary = { total: number; pending_review: number; by_event_code: Record<string, number>; by_quality_tier: Record<string, number>; by_model: Record<string, number>; earliest_onset_at?: string; latest_onset_at?: string };
    ...
    <main>
      ...
      <div className="content">
        ...
        <AnimatePresence mode="wait"><motion.div key={page} className={page === 'overview' ? 'overview-page' : 'secondary-page'} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .12 }}>
          ...
          {page === 'models' && <Models tx={tx} view={subPage.models} lang={lang} />}
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !historyResponse.ok || !backfillResponse.ok || !candidatesResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${historyResponse.status}/${backfillResponse.status}/${candidatesResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12"><CardHead code="HISTORICAL FAULT CANDIDATES" title={tx('历史 GPU 故障候选', 'Historical GPU Fault Candidates')} action={<div className="table-actions"><Badge value={`${historicalCandidateSummary?.pending_review || 0} PENDING REVIEW`} kind={(historicalCandidateSummary?.pending_review || 0) > 0 ? 'warning' : 'neutral'} /><button className="link" type="button" disabled={historyBackfillRunning || latestBackfill?.status === 'running' || latestBackfill?.status === 'queued'} onClick={() => void runAlertBackfill()}>{historyBackfillRunning ? tx('重建中…', 'Backfilling…') : tx('重建历史候选', 'Backfill candidates')}</button></div>} /><p className="node-access-note">{tx('按 7 天分块、1 分钟步长扫描历史 ALERTS 起点，并把一小时内同 GPU、同 XID 合并为一个待审核 episode。候选事件不会自动成为确认训练标签。', 'Scan historical ALERTS onsets in seven-day chunks at one-minute resolution and merge the same GPU/XID within one hour into one reviewable episode. Candidates are never promoted automatically to confirmed training labels.')}</p><div className="issue-categories quality-ledger"><div><b>{tx('候选总数', 'Candidates')}</b><strong>{historicalCandidateSummary?.total || 0}</strong><small>REVIEWABLE EPISODES</small></div><div><b>XID 79</b><strong>{historicalCandidateSummary?.by_event_code?.['79'] || 0}</strong><small>FALLEN OFF BUS</small></div><div><b>XID 94</b><strong>{historicalCandidateSummary?.by_event_code?.['94'] || 0}</strong><small>CONTAINED ECC / WEAK</small></div><div><b>{tx('回填进度', 'Backfill Progress')}</b><strong>{latestBackfill ? `${latestBackfill.chunks_completed}/${latestBackfill.chunks_total}` : '—'}</strong><small>{latestBackfill?.status?.toUpperCase() || 'NOT STARTED'} · {latestBackfill?.query_version || 'gpu-alert-onset-v1'}</small></div></div><div className="table-wrap"><table className="prediction-label-table"><thead><tr><th>{tx('发生时间', 'Onset')}</th><th>{tx('节点 / GPU', 'Node / GPU')}</th><th>{tx('型号 / PCIe', 'Model / PCIe')}</th><th>{tx('事件', 'Event')}</th><th>{tx('质量与审核', 'Quality / Review')}</th><th>{tx('历史来源', 'Historical Source')}</th></tr></thead><tbody>{historicalCandidates.map(candidate => <tr key={candidate.id}><td>{time(candidate.onset_at, lang)}</td><td><b>{candidate.node_ip || candidate.hostname || '—'}</b><small>{candidate.gpu_uuid}</small></td><td>{candidate.model_name || '—'}<small>{candidate.pci_bus_id || '—'}</small></td><td><code>{candidate.event_type}</code><small>{candidate.event_message || `XID ${candidate.event_code}`}</small></td><td><Badge value={candidate.quality_tier.toUpperCase()} kind={candidate.quality_tier === 'strong_proxy' ? 'warning' : 'info'} /><small>{candidate.review_status.toUpperCase()}</small></td><td><small>ALERTS · {candidate.source_alert_name}</small><small>{candidate.signal_samples} ONSET SAMPLES</small></td></tr>)}</tbody></table>{historicalCandidates.length === 0 && <Empty tx={tx} title={tx('尚未执行历史候选重建', 'Historical candidate backfill has not run')} />}</div></Card>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.4.0',
    status: tx('历史故障候选重建', 'HISTORICAL FAULT RECONSTRUCTION'),
    desc: tx(
      '在部署节点本地从历史 ALERTS 重建 GPU XID 故障候选，按事件去重并进入人工审核队列；候选不会自动成为确认训练标签，当前仍不输出故障概率。',
      'Reconstruct GPU XID fault candidates locally from historical ALERTS, deduplicate them into episodes, and place them in a human-review queue. Candidates never become confirmed labels automatically, and no failure probability is emitted yet.',
    ),
    history: [
