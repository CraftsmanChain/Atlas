...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    { id: 'asset', name: tx('资产对账', 'Asset Reconciliation'), version: 'v0.3.0', status: tx('在线资产权威边界', 'LIVE-ASSET AUTHORITY'), desc: tx('以 LXOP 双接口实时在用资产为事实来源；仅 on / 已上架使用中进入 GPU 监控域，状态、IP 或接口成员变化在每轮 10 分钟单飞对账中生效。', 'Live in-use assets from both LXOP APIs are authoritative. Only on / 已上架使用中 assets enter the GPU monitoring domain; state, IP, and API membership changes apply in every single-flight 10-minute reconciliation.'), history: [tx('v0.3.0 · 在线状态白名单成为统一监控边界，10 分钟内退休非在用或已移除节点并失效当前评分', 'v0.3.0 · in-use state allowlist became the monitoring boundary; non-in-use or removed nodes retire and current scores invalidate within 10 minutes'), tx('v0.2.1 · GPU 身份与 Target 合并为 10 分钟单飞对账，恢复状态同批更新', 'v0.2.1 · GPU identity and targets use one 10-minute single-flight reconciliation with same-run recovery'), tx('v0.2.0 · 明确 LXOPEasier 资产事实源；接口同步待接入', 'v0.2.0 · LXOPEasier established as source of truth; API sync pending'), tx('v0.1.2 · Target、身份增量与每日全量对账', 'v0.1.2 · target/identity incremental and daily full reconciliation'), tx('v0.1.1 · GPU 主机范围过滤与历史 UUID 恢复', 'v0.1.1 · GPU host filtering and historical UUID recovery')] },
