    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('特征漂移', 'Feature drift')}</b><strong>{validationReadiness.feature_drift_status.toUpperCase()}</strong><small>{validationReadiness.feature_drift_feature_columns} COLS · {validationReadiness.feature_drift_feature_distributions} DIST · {validationReadiness.feature_drift_psi_status.toUpperCase()}</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.17.1',
    status: tx('特征分布快照可观测', 'FEATURE DISTRIBUTION SNAPSHOT VISIBILITY'),
    desc: tx(
