...
function Gpus({ tx, view, assets, models, loading, inventoryError, healthScores, healthSummary, lang }: { tx: Tx; view: string; assets: GPUAsset[]; models: { model: string; short: string; count: number }[]; loading: boolean; inventoryError: boolean; healthScores: GPUHealthScore[]; healthSummary: HealthSummary | null; lang: string }) {
  ...
  if (view === 'health') {
    ...
    return <div className="grid">
      {[[tx('平均健康分', 'Average score'), healthSummary ? healthSummary.average_score.toFixed(1) : '—', `${healthSummary?.scored || 0} SCORED`, CircleGauge], [tx('风险卡', 'At-risk GPUs'), String(riskCount), 'ATTENTION + WARNING + CRITICAL', ShieldAlert], [tx('数据未知', 'Unknown'), String(healthSummary?.unknown ?? '—'), 'SCORE = NULL', AlertTriangle], [tx('规则命中', 'Rule hits'), String(healthSummary?.latest_run?.rule_hit_count ?? '—'), healthSummary?.latest_run?.rule_version || 'WAITING', Activity], [tx('降级取值', 'Fallback GPUs'), String(healthSummary?.fallback_gpus ?? '—'), 'GPU_EXPORTER FALLBACK', Database], [tx('持续双源偏差', 'Persistent mismatch'), String(healthSummary?.inconsistent_gpus ?? '—'), `${healthSummary?.consistency_candidate_gpus ?? 0} CANDIDATES`, AlertTriangle]].map(([label, value, note, Icon]) => { const I = Icon as typeof Cpu; return <Card className="metric" key={String(label)}><I size={18} /><span>{String(label)}</span><strong>{String(value)}</strong><small>{String(note)}</small></Card>; })}
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'health', name: tx('硬件健康评分', 'Hardware Health Scoring'), version: 'v1.1.0', status: tx('双源 GPU 基线', 'DUAL-SOURCE GPU BASELINE'), desc: tx('以 DCGM 为主、gpu_exporter 补充与降级备用，结合可解释规则、六个健康领域、来源血缘和数据置信度输出 GPU 当前健康状态。', 'Score current GPU health with DCGM primary and gpu_exporter supplements/fallbacks, using explainable rules, six health domains, source lineage and data confidence.'), history: [tx('v1.1.0 · gpu_exporter 补充特征、DCGM 优先降级备用、双源一致性与置信度降级', 'v1.1.0 · gpu_exporter supplements, DCGM-first fallback, consistency checks and confidence degradation'), tx('v1.0.0 · GPU 六领域规则评分与 A–D 置信度', 'v1.0.0 · six-domain GPU rules and A–D confidence')] },
