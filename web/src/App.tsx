...
type ValidationReadinessReport = { version: string; framework_version: string; mode: string; status: string; readiness_sha256: string; label_gate_status: string; label_manifest_version: string; label_manifest_sha256: string; evidence_bundle_version: string; evidence_bundle_sha256: string; evidence_positive_labels: number; evidence_excluded_labels: number; outcome_report_version: string; outcome_stability_status: string; outcome_maturity: { total: number; matured: number; pending: number; censored: number; matured_ratio: number; probability_scored: number; node_eligible: number }; challenger_version: string; challenger_status: string; challenger_confidence_status: string; seven_day_rows: number; seven_day_nodes: number; seven_day_positives: number; seven_day: Array<{ policy: string; description: string; rows: number; nodes: number; positives: number; ranking_at_k?: RankingAtK[] }>; blocking_reasons: string[]; recommended_next_run: string[]; generated_at: string };
...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, accuracyResponse, outcomeReportResponse, governanceResponse, labelManifestResponse, evidenceBundleResponse, challengerResponse, validationReadinessResponse, outcomesResponse, resultsResponse, parityResponse, replayResponse, coverageResponse, shadowResponse, historyResponse, backfillResponse, candidatesResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/accuracy'), fetch('/api/v1/prediction/outcome-report'), fetch('/api/v1/prediction/model-governance'), fetch('/api/v1/prediction/label-manifest'), fetch('/api/v1/prediction/evidence-bundle'), fetch('/api/v1/prediction/hearank-challenger'), fetch('/api/v1/prediction/validation-readiness'), fetch('/api/v1/prediction/outcomes'), fetch('/api/v1/prediction/results'), fetch('/api/v1/prediction/feature-parity'), fetch('/api/v1/prediction/history/feature-replays'), fetch('/api/v1/prediction/history/live-coverage'), fetch('/api/v1/prediction/history/shadow-scoring'), fetch('/api/v1/prediction/history/sources'), fetch('/api/v1/prediction/history/backfills'), fetch('/api/v1/prediction/history/candidates')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !accuracyResponse.ok || !outcomeReportResponse.ok || !governanceResponse.ok || !labelManifestResponse.ok || !evidenceBundleResponse.ok || !challengerResponse.ok || !validationReadinessResponse.ok || !outcomesResponse.ok || !resultsResponse.ok || !parityResponse.ok || !replayResponse.ok || !coverageResponse.ok || !shadowResponse.ok || !historyResponse.ok || !backfillResponse.ok || !candidatesResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${accuracyResponse.status}/${outcomeReportResponse.status}/${governanceResponse.status}/${labelManifestResponse.status}/${evidenceBundleResponse.status}/${challengerResponse.status}/${validationReadinessResponse.status}/${outcomesResponse.status}/${resultsResponse.status}/${parityResponse.status}/${replayResponse.status}/${coverageResponse.status}/${shadowResponse.status}/${historyResponse.status}/${backfillResponse.status}/${candidatesResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, accuracyPayload, outcomeReportPayload, governancePayload, labelManifestPayload, evidenceBundlePayload, challengerPayload, validationReadinessPayload, outcomesPayload, resultsPayload, parityPayload, replayPayload, coveragePayload, shadowPayload, historyPayload, backfillPayload, candidatesPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), accuracyResponse.json(), outcomeReportResponse.json(), governanceResponse.json(), labelManifestResponse.json(), evidenceBundleResponse.json(), challengerResponse.json(), validationReadinessResponse.json(), outcomesResponse.json(), resultsResponse.json(), parityResponse.json(), replayResponse.json(), coverageResponse.json(), shadowResponse.json(), historyResponse.json(), backfillResponse.json(), candidatesResponse.json()]);
        if (!cancelled) {
  ...
  if (view === 'prediction') {
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="prediction-contract">
          <span><b>{tx('绑定报告', 'Bound reports')}</b>{validationReadiness.outcome_report_version}<small>{validationReadiness.challenger_version} · {validationReadiness.evidence_bundle_version}</small></span>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.12.1',
    status: tx('验证证据绑定', 'VALIDATION EVIDENCE BINDING'),
    desc: tx(
