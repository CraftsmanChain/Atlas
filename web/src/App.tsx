...
function AssetStatistics({ tx, lang }: { tx: Tx; lang: string }) {
  ...
  return <div className="grid asset-statistics">
    <Card className="span-12 capability-note"><Layers3 size={19} /><div><CardHead code="LXOP CROSS VALIDATION" title={tx('实时资产分类与双接口交叉验证', 'Live Asset Classification & Cross-validation')} /><p>{tx('按 SN 优先、IP 补充关联同一设备；资产设备接口的 type 作为分类事实，运维主机独有设备使用型号证据补充分组。点击图形可下钻到具体节点。', 'Link the same device by serial number first and IP second. Asset-machine type is authoritative; ops-only devices use model evidence. Click any chart to drill into devices.')}</p></div><Badge value={loading ? 'SYNCING' : `${summary?.total || 0} ENTITIES`} kind={error ? 'warning' : 'info'} /></Card>
