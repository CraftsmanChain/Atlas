...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  useEffect(() => {
    ...
    const reference = sameMatrix.find(build => build.algorithm === 'logistic_regression' && policyOf(build) === 'all_available_windows');
    const challengers = sameMatrix.filter(build => build.id !== reference?.id && !sameMatrix.some(earlier => earlier.id > build.id && `${earlier.algorithm}:${policyOf(earlier)}` === `${build.algorithm}:${policyOf(build)}`));
  ...
  const trainBaselineModels = async (algorithm: 'logistic_regression' | 'gradient_boosted_stumps' | 'anomaly_filtered_logistic' | 'anomaly_augmented_logistic' = 'logistic_regression', featureWindowPolicy: 'all_available_windows' | 'max_24h' | 'max_3d' | 'max_7d' = 'all_available_windows') => {
