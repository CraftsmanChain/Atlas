...
function Models({ tx, view, lang }: { tx: Tx; view: string; lang: string }) {
  ...
  useEffect(() => {
    ...
    const loadPrediction = async () => {
      try {
        const [overviewResponse, readinessResponse, labelsResponse, historyResponse] = await Promise.all([fetch('/api/v1/prediction/overview'), fetch('/api/v1/prediction/readiness'), fetch('/api/v1/prediction/labels'), fetch('/api/v1/prediction/history/sources')]);
        if (!overviewResponse.ok || !readinessResponse.ok || !labelsResponse.ok || !historyResponse.ok) throw new Error(`HTTP ${overviewResponse.status}/${readinessResponse.status}/${labelsResponse.status}/${historyResponse.status}`);
        const [overviewPayload, readinessPayload, labelsPayload, historyPayload] = await Promise.all([overviewResponse.json(), readinessResponse.json(), labelsResponse.json(), historyResponse.json()]);
        if (!cancelled) {
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.3.0',
    status: tx('远端历史数据审计', 'REMOTE HISTORY AUDIT'),
    desc: tx(
      '在 Atlas 部署节点本地审计 Prometheus/VM 历史范围、指标族、采样和 GPU 覆盖，为事件中心回填与训练数据集构建提供只读数据源底座。当前仍不输出故障概率。',
      'Audit Prometheus/VM history range, metric families, cadence, and GPU coverage locally on the Atlas deployment node, providing a read-only source foundation for event-centered backfills and training datasets. No failure probability is emitted yet.',
    ),
    history: [
