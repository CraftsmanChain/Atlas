...
type PredictionFeatureDistributionArchiveMeta = { archive_version?: string; archive_sha256?: string; scope?: { name: string; status: string; limit: number; source_baseline_build_id?: number; model_spec_id?: number; model_key?: string; model_version?: string; distribution_role?: string; feature_contract_version?: string; scope_model_name?: string }; snapshot_count?: number; training_snapshot_count?: number; live_shadow_snapshot_count?: number; baseline_count?: number; feature_count?: number; latest_observed_at?: string; blocking_reasons?: string[]; raw_samples_stored?: boolean; scoring_allowed?: boolean; alerts_emitted?: boolean; actions_executed?: boolean };
    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('分布归档', 'Distribution archive')}</b><strong>{validationReadiness.feature_distribution_archive_sha256?.slice(0, 16) || '—'}</strong><small>{validationReadiness.feature_distribution_archive_scope?.status?.toUpperCase() || 'NO SCOPE'} · {validationReadiness.feature_distribution_training_snapshots} TRAIN · {validationReadiness.feature_distribution_live_shadow_snapshots} LIVE</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.18.7',
    status: tx('就绪门绑定分布归档', 'READINESS BINDS DISTRIBUTION ARCHIVE'),
    desc: tx(
