    ...
    return <div className="grid prediction-page">
      ...
      {warningTab === 'feedback-pack' ? <Card id="prediction-feedback-package" className="span-12">
        ...
        <div className="issue-categories quality-ledger compact-ledger">
          <div><b>{tx('最新特征请求', 'Latest feature request')}</b><strong>{compactWorkflowStatus(latestFeatureStatus)}</strong><small>{latestFeedbackFeatureRequest?.version || 'manual-feedback-feature-request-v4'}</small></div>
      ...
      {shadowModelCandidates.length > 0 ? <Card id="prediction-shadow-runtime" className="span-12">
        ...
        <p className="node-access-note">{tx('人工触发的只读前瞻评分：每批重新验证模型 SHA256、当前 24 小时数据覆盖和 70 列特征契约，并为已评分 GPU 生成 live-shadow 逐特征聚合分布快照。超阈值只是待验证影子候选，不是硬件高危判定；超阈值比例、中位数或节点聚集异常会阻断周期调度。', 'Manually triggered read-only prospective scoring. Every run rechecks model SHA256, current trailing-24h coverage, and all 70 columns, then materializes live-shadow aggregate per-feature distribution snapshots for scored GPUs. Above-threshold means an unvalidated shadow candidate, not confirmed hardware risk; abnormal hit ratio, median, or node clustering blocks periodic scheduling.')}</p>
