...
type ManualFeedbackFeatureRequestBuild = { id: number; request_key: string; version: string; status: string; source_key: string; source_manifest_version: string; source_manifest_sha256: string; feature_contract_version: string; lookback_minutes: number; query_step_seconds: number; hardware_feedback_requests: number; training_eligible_requests: number; pack_ready_requests: number; warning_reviewed_requests: number; warning_miss_requests: number; blocked_requests: number; window_count: number; metric_family_count: number; metric_families: string[]; blocking_reasons: string[]; output_dir: string; manifest_path?: string; manifest_sha256?: string; no_raw_telemetry_stored: boolean; no_alert_emitted: boolean; no_action_executed: boolean; error_message?: string; started_at: string; finished_at?: string };
...
function Models({ tx, view, lang, assets }: { tx: Tx; view: string; lang: string; assets: GPUAsset[] }) {
  ...
  if (isPredictionView) {
    ...
    return <div className="grid prediction-page">
      ...
      <Card className="span-12">
        <CardHead code="MANUAL FEEDBACK FEATURE REQUESTS" title={tx('人工故障反馈特征请求', 'Manual Fault-feedback Feature Requests')} action={<div className="table-actions"><Badge value="MANIFEST ONLY / READ-ONLY" kind="info" /><button className="link" type="button" disabled={feedbackFeatureRequestStarting} onClick={() => void buildManualFeedbackFeatureRequestManifest()}>{feedbackFeatureRequestStarting ? tx('生成中…', 'Building…') : tx('生成请求 Manifest', 'Build request manifest')}</button></div>} />
        <p className="node-access-note">{tx('把 pack-ready、身份已解析、预警已复核的人工硬件故障反馈固化为离线 worker 可消费的特征提取请求清单。该步骤只写 manifest 和 SHA，不读取或复制原始历史遥测，不发告警也不执行动作。', 'Materializes pack-ready, identity-resolved, warning-reviewed manual hardware-fault feedback into a feature-extraction request manifest consumable by a later offline worker. This step writes only manifest metadata and SHA; it does not read or copy raw historical telemetry, emit alerts, or execute actions.')}</p>
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('数据包 / 复核', 'Pack / review')}</b><strong>{latestFeedbackFeatureRequest ? `${latestFeedbackFeatureRequest.pack_ready_requests}/${latestFeedbackFeatureRequest.warning_reviewed_requests}` : '—'}</strong><small>{latestFeedbackFeatureRequest?.warning_miss_requests || 0} WARNING MISSES</small></div>
        ...
        {latestFeedbackFeatureRequest ? <div className="prediction-contract"><span><b>SOURCE MANIFEST</b>{latestFeedbackFeatureRequest.source_manifest_sha256?.slice(0, 16) || '—'}</span><span><b>FEATURE CONTRACT</b>{latestFeedbackFeatureRequest.feature_contract_version}</span><span><b>LOOKBACK / STEP</b>{latestFeedbackFeatureRequest.lookback_minutes}m / {latestFeedbackFeatureRequest.query_step_seconds}s</span><span><b>OUTPUT</b>{latestFeedbackFeatureRequest.output_dir}</span><span><b>SHA256</b>{latestFeedbackFeatureRequest.manifest_sha256 || latestFeedbackFeatureRequest.error_message || latestFeedbackFeatureRequest.blocking_reasons?.[0] || 'PENDING'}</span></div> : <Empty tx={tx} title={tx('等待从人工故障反馈生成离线特征请求清单', 'Waiting to build an offline feature request manifest from manual fault feedback')} />}
      </Card>
      <Card className="span-12">
