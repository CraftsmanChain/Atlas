...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.6.0', status: tx('链路连续性基线', 'PIPELINE CONTINUITY'), desc: tx('联合识别每卡样本存在率、最大间隔、UUID 波动及 DCGM Target 抓取成功率、样本量和耗时变化；异常进入数据统计并自动恢复。', 'Combine per-GPU presence, maximum gap and UUID flaps with DCGM target success, sample-volume and duration ratios; findings enter Data Statistics and auto-recover.'), history: [tx('v0.6.0 · 新增最大间隔、UUID 波动与 Target 抓取质量判定', 'v0.6.0 · maximum gap, UUID flaps and target scrape-quality decisions'), tx('v0.5.0 · 新增每卡 1h 样本存在率、当前样本年龄、连续性页面与问题闭环', 'v0.5.0 · per-GPU 1h presence, current sample age, continuity UI and issue lifecycle'), tx('v0.4.3 · 10 分钟采集周期、非重入调度与采集超时状态', 'v0.4.3 · 10-minute cadence, non-reentrant scheduling and collection-overdue status'), tx('v0.4.2 · Target 问题严格限定当前 GPU 节点生命周期', 'v0.4.2 · target issues are strictly scoped to the current GPU-node lifecycle')] },
    { id: 'health', name: tx('硬件健康评分', 'Hardware Health Scoring'), version: 'v1.4.0', status: tx('结构质量增强', 'STRUCTURAL QUALITY PLUS'), desc: tx('评分快照携带样本连续性、UUID 波动和采集链路特征；结构异常进入数据质量而不误判为硬件故障。', 'Score snapshots carry sample continuity, UUID flap and collection-path features so structural anomalies enter data quality instead of hardware faults.'), history: [tx('v1.4.0 · 接入最大间隔、UUID 波动和 DCGM Target 抓取质量', 'v1.4.0 · maximum gap, UUID flaps and DCGM target scrape quality'), tx('v1.3.0 · 健康快照接入每卡结构性可观测性特征', 'v1.3.0 · health snapshots consume per-GPU structural observability features'), tx('v1.2.0 · Correctable Row Remap 改为 1h/24h 增长判定，稳定累计值只观察', 'v1.2.0 · correctable row remaps use 1h/24h growth; stable totals are observation-only'), tx('v1.1.3 · 健康监控改为 10 分钟单飞评估并公开超时状态', 'v1.1.3 · health monitoring uses 10-minute single-flight evaluation with overdue visibility')] },
    ...
    { id: 'feature', name: tx('统一特征目录', 'Unified Feature Catalog'), version: 'v1.4.0', status: tx('结构契约增强', 'STRUCTURAL CONTRACT PLUS'), desc: tx('35 个规范特征和 53 个源查询统一服务健康评分、PyOD、风险排序、监督预测和性能衰减，覆盖 GPU 时序及 Target 采集链路。', 'Thirty-five canonical features and 53 source queries serve health, PyOD, risk ranking, prediction and degradation across GPU series and target collection paths.'), history: [tx('v1.4.0 · 新增最大间隔、UUID 波动和三项 Target 抓取结构特征', 'v1.4.0 · maximum gap, UUID flap and three target scrape features'), tx('v1.3.0 · 新增样本数、1h 存在率和当前样本年龄结构特征', 'v1.3.0 · sample count, 1h presence and current sample-age features'), tx('v1.2.0 · 27 个规范健康特征、45 个源查询，新增 Correctable Row Remap 增量', 'v1.2.0 · 27 canonical health features and 45 source queries with correctable-row-remap deltas'), tx('v1.1.0 · 25 个规范健康特征、41 个源查询、来源血缘和回退语义', 'v1.1.0 · 25 canonical health features, 41 source queries, lineage and fallback semantics')] },
