    ...
    return <div className="grid prediction-page">
      ...
      {validationReadiness ? <Card className="span-12">
        ...
        <div className="issue-categories quality-ledger">
          ...
          <div><b>{tx('成熟结果', 'Mature outcomes')}</b><strong>{validationReadiness.outcome_maturity.matured} / {validationReadiness.outcome_maturity.total}</strong><small>{pct(validationReadiness.outcome_maturity.matured_ratio)} MATURED · {validationReadiness.outcome_maturity.probability_scored} SCORED</small></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = baseModules.map(module => module.id === 'prediction' ? {
    ...module,
    version: 'v0.10.0',
    status: tx('HeaRank 样本置信度门', 'HEARANK SAMPLE CONFIDENCE GATE'),
    desc: tx(
