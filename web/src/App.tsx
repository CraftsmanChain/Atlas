...
export default function App() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [s, df, f, fs, ga, ct, sr, ac, hs, hg, fes, is, pc, ds, dc] = await Promise.all([
        ...
        fetch('/api/v1/degradation/summary'), fetch('/api/v1/degradation/candidates'),
      ]);
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.4.3', status: tx('双源质量基线', 'DUAL-SOURCE BASELINE'), desc: tx('识别当前 GPU 资产范围内的 Target 异常、指标缺失或陈旧、覆盖不足、身份冲突和采集链路问题；页面提示超过 10 分钟仍未完成的采集。', 'Detect target failures, missing or stale metrics, coverage gaps, identity conflicts and collection-path issues within the current GPU asset scope; the UI reports collection runs exceeding 10 minutes.'), history: [tx('v0.4.3 · 10 分钟采集周期、非重入调度与采集超时状态', 'v0.4.3 · 10-minute cadence, non-reentrant scheduling and collection-overdue status'), tx('v0.4.2 · Target 问题严格限定当前 GPU 节点生命周期', 'v0.4.2 · target issues are strictly scoped to the current GPU-node lifecycle'), tx('v0.4.1 · 双源数值差异改为审计信息，不再生成数据质量问题', 'v0.4.1 · cross-source value differences became audit-only and no longer create data-quality issues'), tx('v0.4.0 · DCGM 与 gpu_exporter 规范化一致性观察', 'v0.4.0 · normalized DCGM/gpu_exporter consistency observations')] },
    { id: 'health', name: tx('硬件健康评分', 'Hardware Health Scoring'), version: 'v1.2.0', status: tx('双源 GPU 基线', 'DUAL-SOURCE GPU BASELINE'), desc: tx('以 DCGM 为主、gpu_exporter 补充与降级备用；Correctable Row Remap 仅在近期快速增长时扣分，平台只告警与记录，不操作任务或节点。', 'Use DCGM as primary and gpu_exporter as supplement/fallback. Correctable row remaps affect scores only when recent growth is rapid; Atlas alerts and records without operating workloads or nodes.'), history: [tx('v1.2.0 · Correctable Row Remap 改为 1h/24h 增长判定，稳定累计值只观察', 'v1.2.0 · correctable row remaps use 1h/24h growth; stable totals are observation-only'), tx('v1.1.3 · 健康监控改为 10 分钟单飞评估并公开超时状态', 'v1.1.3 · health monitoring uses 10-minute single-flight evaluation with overdue visibility'), tx('v1.1.2 · 双源差异仅审计，健康页精简为一行四项核心指标', 'v1.1.2 · source differences are audit-only and the health page now shows one row of four core metrics'), tx('v1.1.1 · 明确已评分平均分口径', 'v1.1.1 · clarified the scored-average scope'), tx('v1.1.0 · gpu_exporter 补充特征与 DCGM 优先降级备用', 'v1.1.0 · gpu_exporter supplements and DCGM-first fallback')] },
    ...
    { id: 'feature', name: tx('统一特征目录', 'Unified Feature Catalog'), version: 'v1.2.0', status: tx('趋势契约完成', 'TREND CONTRACT'), desc: tx('为健康评分、PyOD、风险排序、监督预测和性能衰减提供版本化特征契约，并新增 Correctable Row Remap 的 1h/24h 增量时间语义。', 'Provide versioned feature contracts for health, PyOD, risk ranking, prediction and degradation, including 1h/24h correctable-row-remap trend semantics.'), history: [tx('v1.2.0 · 27 个规范健康特征、45 个源查询，新增 Correctable Row Remap 增量', 'v1.2.0 · 27 canonical health features and 45 source queries with correctable-row-remap deltas'), tx('v1.1.0 · 25 个规范健康特征、41 个源查询、来源血缘和回退语义', 'v1.1.0 · 25 canonical health features, 41 source queries, lineage and fallback semantics'), tx('v1.0.0 · 注册/读取 API、20 个健康核心特征和 snapshot 版本 manifest', 'v1.0.0 · registration/read APIs, 20 core health features and snapshot version manifests')] },
