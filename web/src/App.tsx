...
const subPages = (page: PageId, tx: Tx): SubPage[] => ({
  ...
  quality: [{ id: 'targets', label: tx('采集覆盖', 'Target Coverage') }, { id: 'continuity', label: tx('指标连续性', 'Metric Continuity') }, { id: 'issues', label: tx('问题统计', 'Issue Statistics') }, { id: 'identity', label: tx('身份与带外', 'Identity & BMC') }, { id: 'audit', label: tx('同步审计', 'Sync Audit') }],
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'feature', name: tx('统一特征目录', 'Unified Feature Catalog'), version: 'v1.6.0', status: tx('全量规则已发布', 'FLEET RULE LIVE'), desc: tx('35 个在线健康特征、53 个源查询和 1 个已发布的 metric-family recording-rule 特征；规则覆盖全量 GPU，仅服务异常检测、风险排序与预测，不重复计入健康分。', 'Thirty-five online health features, 53 source queries and one published metric-family recording-rule feature. It covers the GPU fleet for anomaly detection, risk ranking and prediction, without duplicate health scoring.'), history: [tx('v1.6.0 · metric-family 规则提升并发布为全量正式特征，保持不参与健康评分', 'v1.6.0 · promoted and published metric-family rule as a production fleet feature while keeping it out of health scoring'), tx('v1.5.0 · metric-family 单节点 canary 配置、shadow 契约与范围保护', 'v1.5.0 · single-node metric-family canary config, shadow contract and scope guard'), tx('v1.4.0 · 新增最大间隔、UUID 波动和三项 Target 抓取结构特征', 'v1.4.0 · maximum gap, UUID flap and three target scrape features'), tx('v1.3.0 · 新增样本数、1h 存在率和当前样本年龄结构特征', 'v1.3.0 · sample count, 1h presence and current sample-age features')] },
    ...
    { id: 'issue', name: tx('数据统计', 'Data Statistics'), version: 'v0.5.0', status: tx('训练数据基线', 'TRAINING DATA BASELINE'), desc: tx('在统一分类统计和人工处置台账之上，输出经过完整性审核、直接标识脱敏和实体隔离划分的训练/评估数据集。', 'Build on classified statistics and human resolution records to export training/evaluation data with completeness gates, direct-identifier redaction and entity-isolated splits.'), history: [tx('v0.5.0 · atlas-issue-training-v1 增加稳定快照、默认脱敏、质量门、训练/评估划分和页面导出', 'v0.5.0 · atlas-issue-training-v1 added stable snapshots, default redaction, quality gates, train/evaluation splits and UI export'), tx('v0.4.0 · 新增按分类的解决、遗留和当前检测统计', 'v0.4.0 · resolved, remaining and active counts by category'), tx('v0.3.0 · 更名为数据统计，硬件故障迁入告警中心且保留底层处置台账', 'v0.3.0 · renamed Data Statistics; hardware faults moved to Alert Center with resolution records retained'), tx('v0.2.3 · 恢复节点的 GPU/Target 状态同批更新并自动退出遗留统计', 'v0.2.3 · recovered GPU/target state updates in one run and automatically leaves remaining totals'), tx('v0.2.2 · 退休节点历史 Target 问题自动清除并退出活跃统计', 'v0.2.2 · historical target issues on retired nodes are automatically cleared and removed from active analytics')] },
