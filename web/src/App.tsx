...
type NodeAccessOverview = {
  ...
  no_arbitrary_shell: boolean; no_change_executed: boolean; encryption_ready: boolean; management_ready: boolean; secure_write_only: boolean;
...
function Quality({ tx, view, targets, summary, issueSummary, inventoryError, syncRuns, assetChanges, telemetry, telemetrySummary, nodeAccess, reloadNodeAccess, openIssues, lang }: { tx: Tx; view: string; targets: CollectorTarget[]; summary: FleetSummary | null; issueSummary: IssueSummary | null; inventoryError: boolean; syncRuns: SyncRun[]; assetChanges: AssetChange[]; telemetry: TelemetryQualityItem[]; telemetrySummary: TelemetryQualitySummary | null; nodeAccess: NodeAccessOverview | null; reloadNodeAccess: () => void; openIssues: () => void; lang: string }) {
  ...
  const credentialTransportReady = window.location.protocol === 'https:' || ['localhost', '127.0.0.1', '::1'].includes(window.location.hostname);
  ...
  if (view === 'node-access') {
    ...
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.2.0'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('所有节点信息收集都必须使用已注册的 Skill 命令 ID、固定参数和资源预算。v0.2 已加入平台加密凭据字典，但仍未启用远程连接或命令执行。', 'All node evidence collection must use registered Skill command IDs, fixed parameters, and resource budgets. v0.2 adds a platform-managed encrypted credential dictionary, while remote connections and command execution remain disabled.')}</p></div></div>
      ...
      <Card className="span-12">
        ...
        <div className="credential-layout">
          <form className="credential-form" onSubmit={event => { event.preventDefault(); void saveCredential(); }}>
            <div className="credential-form-head"><div><b>{tx('新增凭据配置', 'Add credential profile')}</b><small>{credentialTransportReady ? tx('管理口令仅保留在当前页面内存', 'Management token remains only in this page memory') : tx('当前 HTTP 地址只读；请使用 HTTPS 或 SSH 本地端口转发', 'This HTTP address is read-only; use HTTPS or an SSH local port forward')}</small></div><Badge value={!credentialTransportReady ? 'SECURE TRANSPORT REQUIRED' : nodeAccess?.management_ready ? 'WRITE PROTECTED' : 'MANAGEMENT OFFLINE'} kind={credentialTransportReady && nodeAccess?.management_ready ? 'info' : 'warning'} /></div>
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'node-access', name: tx('节点证据 Skill', 'Node Evidence Skill'), version: 'v0.2.0', status: tx('加密凭据基线', 'ENCRYPTED CREDENTIAL BASELINE'), desc: tx('以版本化 Skill 统一声明节点信息采集、命令白名单、资源预算和人工审批边界；平台支持 AES-256-GCM 加密凭据字典、管理口令写保护及 A→B 轮换。当前仍不建立远程连接。', 'A versioned Skill defines node evidence collection, registered commands, resource budgets, and human-approval boundaries. The platform now provides an AES-256-GCM encrypted credential dictionary, management-token write protection, and A-to-B rotation. Remote connections remain disabled.'), history: [tx('v0.2.0 · 页面加密录入、密文落库、管理口令写保护、掩码列表与认证前解密', 'v0.2.0 · encrypted page entry, ciphertext persistence, management-token write protection, masked lists, and pre-authentication decryption'), tx('v0.1.0 · Skill 契约、只读/审批命令分级、凭据引用轮换状态机与状态页面', 'v0.1.0 · Skill contract, read-only/approval command classes, credential-reference rotation state machine, and status UI'), tx('v0.1.0 · 禁止明文凭据、任意 Shell、远程连接和执行 API', 'v0.1.0 · plaintext credentials, arbitrary shell, remote connections, and execution APIs are prohibited')] },
