...
type NodeAccessOverview = {
  ...
  no_arbitrary_shell: boolean; no_change_executed: boolean; encryption_ready: boolean; management_ready: boolean; secure_write_only: boolean; insecure_http_allowed: boolean;
  ...
  if (view === 'node-access') {
    ...
    return <div className="grid node-access-page">
      <Card className="span-12 node-access-guard">
        <div><ShieldCheck size={21} /><div><CardHead code={`${nodeAccess?.skill_id || 'atlas-node-evidence'} / ${nodeAccess?.skill_version || 'v0.2.1'}`} title={tx('节点只读证据 Skill', 'Node Read-only Evidence Skill')} /><p>{tx('所有节点信息收集都必须使用已注册的 Skill 命令 ID、固定参数和资源预算。v0.2.1 增加加密凭据字典和显式 HTTP 兼容开关；远程连接和命令执行仍未启用。', 'All node evidence collection must use registered Skill command IDs, fixed parameters, and resource budgets. v0.2.1 adds the encrypted credential dictionary and an explicit HTTP compatibility switch; remote connections and command execution remain disabled.')}</p></div></div>
      ...
      <Card className="span-12">
        ...
        <div className="node-skill-grid">{(nodeAccess?.skills || []).map(skill => <article key={skill.id}><header><code>{skill.id}</code><Badge value={skill.status.toUpperCase()} kind={skill.status === 'safety_baseline' ? 'healthy' : 'info'} /></header><b>{local(skill.purpose)}</b><small>{skill.class.toUpperCase()} · {skill.version}</small></article>)}</div>
      </Card>
      <Card className="span-12">
