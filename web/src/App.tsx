...
type TelemetryQualityItem = { gpu_asset_id: number; gpu_uuid: string; node_ip: string; gpu_index: number; model_name: string; status: string; sample_count_1h?: number; presence_ratio_1h?: number; sample_age_seconds?: number; observed_at: string };
type TelemetryQualitySummary = { total: number; by_status: Record<string, number>; average_presence_ratio_1h: number; max_sample_age_seconds: number; feature_catalog_version: string; evaluation_run_id: number; evaluated_at?: string };
type DegradationSummary = { version: string; mode: string; evaluated_gpus: number; eligible_gpus: number; baseline_ready_gpus: number; candidate_gpus: number; insufficient_gpus: number; minimum_utilization: number; ratio_threshold: number; freshness_sla_seconds: number; by_model: Record<string, number>; latest_observed_at?: string; evaluated_at: string };
type DegradationCandidate = { gpu_asset_id: number; gpu_uuid: string; node_ip: string; gpu_index: number; model_name: string; status: string; metric: string; observed_value: number; baseline_value: number; performance_ratio: number; gpu_utilization: number; peer_count: number; baseline_scope: string; data_confidence: string; evidence: string[]; recommended_action: string; feature_version: string; observed_at: string; evaluated_at: string };
type FleetSummary = {
...
const subPages = (page: PageId, tx: Tx): SubPage[] => ({
  ...
  quality: [{ id: 'targets', label: tx('采集覆盖', 'Target Coverage') }, { id: 'continuity', label: tx('指标连续性', 'Metric Continuity') }, { id: 'identity', label: tx('身份与带外', 'Identity & BMC') }, { id: 'audit', label: tx('同步审计', 'Sync Audit') }],
...
export default function App() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [s, df, f, fs, ga, ct, sr, ac, hs, hg, fes, is, pc, ds, dc, tq] = await Promise.all([
    ...
    <main>
      ...
      <div className="content">
        ...
        <AnimatePresence mode="wait"><motion.div key={page} className={page === 'overview' ? 'overview-page' : 'secondary-page'} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .12 }}>
          ...
          {page === 'quality' && <Quality tx={tx} view={subPage.quality} targets={targets} summary={summary} inventoryError={inventoryError} syncRuns={syncRuns} assetChanges={assetChanges} telemetry={telemetryQuality} telemetrySummary={telemetryQualitySummary} lang={lang} />}
...
function Incidents({ tx, view, rows, ingestionMeta, ingestionPage, ingestionLoading, previousIngestionPage, nextIngestionPage, faultRows, faultMeta, faultPage, faultLoading, previousFaultPage, nextFaultPage, query, setQuery, level, setLevel, failures, select, selectIssue, lang }: { tx: Tx; view: string; rows: Ingestion[]; ingestionMeta: IngestionMeta; ingestionPage: number; ingestionLoading: boolean; previousIngestionPage: () => void; nextIngestionPage: () => void; faultRows: FaultEvent[]; faultMeta: CursorMeta; faultPage: number; faultLoading: boolean; previousFaultPage: () => void; nextFaultPage: () => void; query: string; setQuery: (v: string) => void; level: string; setLevel: (v: string) => void; failures: Failure[]; select: (id: number) => void; selectIssue: (id: number) => void; lang: string }) {
...
function Validations({ tx, view, summary, candidates, lang }: { tx: Tx; view: string; summary: DegradationSummary | null; candidates: DegradationCandidate[]; lang: string }) {
  ...
  const scope = (value: string) => value === 'same_node_model' ? tx('同节点同型号', 'Same node/model') : tx('同型号集群', 'Same-model fleet');
  return <div className="grid">
    ...
    <Card className="span-3 degradation-stat"><small>{tx('基线就绪', 'BASELINE READY')}</small><strong>{summary?.baseline_ready_gpus ?? '—'}</strong><span>{tx('满足同类样本门槛', 'peer threshold met')}</span></Card>
    ...
    <Card className="span-12 degradation-table-card"><CardHead code={summary?.version || 'degradation-v0.1.0'} title={tx('候选明细', 'Candidate Details')} action={<span className="table-observed">{tx('最近观测', 'LATEST OBSERVED')} {time(summary?.latest_observed_at, lang)}</span>} />
    ...
    <Card className="span-12 degradation-method"><CardHead code="METHOD" title={tx('当前判定口径', 'Current Decision Contract')} /><div className="stages"><div><b>01 / ELIGIBILITY</b><h3>{tx('高负载新鲜观测', 'Fresh High-load Observation')}</h3><p>gpu_util_avg_15m ≥ {summary?.minimum_utilization ?? 80}% · confidence A/B · freshness ≤ {Math.round((summary?.freshness_sla_seconds ?? 3600) / 60)}m</p><Badge value="NON-INTRUSIVE" kind="healthy" /></div><ChevronRight /><div><b>02 / BASELINE</b><h3>{tx('稳健同类基线', 'Robust Peer Baseline')}</h3><p>{tx('同节点同型号优先 · 同型号集群兜底 · 中位数', 'same node/model first · fleet fallback · median')}</p><Badge value="FEATURE CATALOG v1" kind="info" /></div><ChevronRight /><div><b>03 / VALIDATE</b><h3>{tx('人工维护复测', 'Human-gated Validation')}</h3><p>DCGM · SuperBench · workload context</p><Badge value="NOT AUTOMATED" kind="warning" /></div></div></Card>
  </div>;
}
function Quality({ tx, view, targets, summary, inventoryError, syncRuns, assetChanges, telemetry, telemetrySummary, lang }: { tx: Tx; view: string; targets: CollectorTarget[]; summary: FleetSummary | null; inventoryError: boolean; syncRuns: SyncRun[]; assetChanges: AssetChange[]; telemetry: TelemetryQualityItem[]; telemetrySummary: TelemetryQualitySummary | null; lang: string }) {
  ...
  if (view === 'continuity') {
    ...
    return <div className="grid">
      <Card className="span-12 capability-note"><Activity size={19} /><div><CardHead code={telemetrySummary?.feature_catalog_version || 'FEATURE CATALOG'} title={tx('GPU 指标连续性', 'GPU Metric Continuity')} /><p>{tx('以现场 15 秒采样周期为基线，统计每张 GPU 最近 1 小时样本存在率和当前样本年龄。该结果只判断遥测可信度，不作为硬件故障或性能异常。', 'Using the observed 15-second scrape cadence, Atlas measures each GPU’s one-hour sample presence and current sample age. This evaluates telemetry trustworthiness, not hardware faults or performance anomalies.')}</p></div><Badge value="READ-ONLY" kind="info" /></Card>
      {[[tx('GPU 总数', 'Total GPUs'), telemetrySummary?.total ?? '—', 'LATEST RUN'], [tx('连续正常', 'Fresh'), telemetrySummary?.by_status.fresh ?? 0, '≥95% · AGE≤60s'], [tx('连续性异常', 'Degraded / Stale'), (telemetrySummary?.by_status.degraded || 0) + (telemetrySummary?.by_status.stale || 0), '<95% OR AGE>60s'], [tx('平均存在率', 'Average Presence'), telemetrySummary ? `${telemetrySummary.average_presence_ratio_1h.toFixed(2)}%` : '—', `MAX AGE ${telemetrySummary?.max_sample_age_seconds.toFixed(1) || '—'}s`]].map(([label, value, note]) => <Card className="metric quality" key={String(label)}><Activity size={17} /><strong>{String(value)}</strong><b>{String(label)}</b><small>{String(note)}</small></Card>)}
      <Card className="span-12"><CardHead code="CONTINUITY ISSUES" title={tx('异常与未知 GPU', 'Degraded and Unknown GPUs')} action={<Badge value={`${problems.length} ITEMS`} kind={problems.length ? 'warning' : 'healthy'} />} /><div className="table-wrap"><table className="target-table"><thead><tr><th>{tx('节点 / GPU', 'Node / GPU')}</th><th>UUID</th><th>{tx('状态', 'Status')}</th><th>{tx('1h 样本', '1h Samples')}</th><th>{tx('1h 存在率', '1h Presence')}</th><th>{tx('样本年龄', 'Sample Age')}</th><th>{tx('观测时间', 'Observed')}</th></tr></thead><tbody>{problems.map(item => <tr key={item.gpu_asset_id}><td><b>{item.node_ip} · GPU {item.gpu_index}</b><small>{item.model_name}</small></td><td><code>{item.gpu_uuid}</code></td><td><Badge value={item.status.toUpperCase()} kind={item.status === 'stale' ? 'danger' : 'warning'} /></td><td>{item.sample_count_1h?.toFixed(0) ?? '—'} / 240</td><td>{item.presence_ratio_1h != null ? `${item.presence_ratio_1h.toFixed(1)}%` : '—'}</td><td>{item.sample_age_seconds != null ? `${item.sample_age_seconds.toFixed(1)}s` : '—'}</td><td>{time(item.observed_at, lang)}</td></tr>)}</tbody></table>{problems.length === 0 && <Empty tx={tx} title={tx('GPU 指标连续性正常', 'GPU metric continuity is healthy')} />}</div></Card>
    </div>;
  }
...
function About({ tx, view, platformConfig, onPlatformConfig }: { tx: Tx; view: string; platformConfig: PlatformConfig; onPlatformConfig: (config: PlatformConfig) => void }) {
  ...
  const modules = [
    ...
    { id: 'quality', name: tx('监控数据质量发现', 'Monitoring Data Quality Detection'), version: 'v0.5.0', status: tx('连续性基线', 'CONTINUITY BASELINE'), desc: tx('识别 Target、指标存在率、样本年龄、覆盖、身份和采集链路问题；每卡连续性异常进入数据统计并在恢复后自动清除。', 'Detect target, metric-presence, sample-age, coverage, identity and collection-path issues. Per-GPU continuity findings enter Data Statistics and auto-clear after recovery.'), history: [tx('v0.5.0 · 新增每卡 1h 样本存在率、当前样本年龄、连续性页面与问题闭环', 'v0.5.0 · per-GPU 1h presence, current sample age, continuity UI and issue lifecycle'), tx('v0.4.3 · 10 分钟采集周期、非重入调度与采集超时状态', 'v0.4.3 · 10-minute cadence, non-reentrant scheduling and collection-overdue status'), tx('v0.4.2 · Target 问题严格限定当前 GPU 节点生命周期', 'v0.4.2 · target issues are strictly scoped to the current GPU-node lifecycle'), tx('v0.4.1 · 双源数值差异改为审计信息，不再生成数据质量问题', 'v0.4.1 · source-value differences became audit-only and no longer create data-quality issues')] },
    { id: 'health', name: tx('硬件健康评分', 'Hardware Health Scoring'), version: 'v1.3.0', status: tx('结构质量基线', 'STRUCTURAL QUALITY BASELINE'), desc: tx('评分快照同时携带指标存在率、样本数和样本年龄，结构异常进入数据质量而不误判为硬件故障。', 'Score snapshots carry metric presence, sample count and age so structural anomalies enter data quality instead of being misclassified as hardware faults.'), history: [tx('v1.3.0 · 健康快照接入每卡结构性可观测性特征', 'v1.3.0 · health snapshots consume per-GPU structural observability features'), tx('v1.2.0 · Correctable Row Remap 改为 1h/24h 增长判定，稳定累计值只观察', 'v1.2.0 · correctable row remaps use 1h/24h growth; stable totals are observation-only'), tx('v1.1.3 · 健康监控改为 10 分钟单飞评估并公开超时状态', 'v1.1.3 · health monitoring uses 10-minute single-flight evaluation with overdue visibility'), tx('v1.1.2 · 双源差异仅审计，健康页精简为一行四项核心指标', 'v1.1.2 · source differences are audit-only and the health page now shows one row of four core metrics')] },
    ...
    { id: 'feature', name: tx('统一特征目录', 'Unified Feature Catalog'), version: 'v1.3.0', status: tx('结构契约完成', 'STRUCTURAL CONTRACT'), desc: tx('30 个规范特征和 48 个源查询统一服务健康评分、PyOD、风险排序、监督预测和性能衰减，并覆盖每卡样本数、存在率与样本年龄。', 'Thirty canonical features and 48 source queries serve health, PyOD, risk ranking, prediction and degradation, including per-GPU sample count, presence and age.'), history: [tx('v1.3.0 · 新增样本数、1h 存在率和当前样本年龄结构特征', 'v1.3.0 · sample count, 1h presence and current sample-age features'), tx('v1.2.0 · 27 个规范健康特征、45 个源查询，新增 Correctable Row Remap 增量', 'v1.2.0 · 27 canonical health features and 45 source queries with correctable-row-remap deltas'), tx('v1.1.0 · 25 个规范健康特征、41 个源查询、来源血缘和回退语义', 'v1.1.0 · 25 canonical health features, 41 source queries, lineage and fallback semantics'), tx('v1.0.0 · 注册/读取 API、20 个健康核心特征和 snapshot 版本 manifest', 'v1.0.0 · registration/read APIs, 20 core health features and snapshot version manifests')] },
    ...
    { id: 'degradation', name: tx('性能衰减识别', 'Performance Degradation Detection'), version: 'v0.1.0', status: tx('被动影子基线', 'PASSIVE SHADOW BASELINE'), desc: tx('消费 Feature Catalog 快照，在高负载条件下以同节点同型号优先、同型号集群兜底的中位数基线识别 SM 时钟衰减候选；不影响健康分，不输出故障概率，显存、PCIe、NVLink 与主动验证待开发。', 'Consume Feature Catalog snapshots and detect SM-clock degradation candidates under high load using same-node/model median first and same-model fleet fallback. It does not affect health scores or emit failure probabilities; memory, PCIe, NVLink and active validation remain pending.'), history: [tx('v0.1.0 · 被动候选 API、同类中位数基线、证据/置信度与影子模式页面', 'v0.1.0 · passive candidate APIs, peer-median baseline, evidence/confidence and shadow-mode UI'), tx('v0.0.1 · 性能特征、SuperBench/DCGM 验证和基线契约', 'v0.0.1 · performance features, SuperBench/DCGM validation and baseline contract'), tx('v0.0.0 · 被动检测与主动验证安全门设计', 'v0.0.0 · passive detection and active-validation safety gates')] },
    { id: 'incident', name: tx('告警中心', 'Alert Center'), version: 'v0.3.0', status: tx('硬件处置基线', 'HARDWARE WORKFLOW BASELINE'), desc: tx('分层管理原始接收记录与 Atlas 硬件告警；硬件告警关联处置台账，支持查看详情并追加根因、方案、解决过程、结果和训练资格。', 'Separate raw ingestion records and Atlas hardware alerts. Hardware alerts link to resolution records with details, root cause, solution, process, result and training eligibility.'), history: [tx('v0.3.0 · 硬件告警关联处置台账并支持详情与人工处置', 'v0.3.0 · hardware alerts link to details and operator resolution workflows'), tx('v0.2.2 · 硬件事件稳定 ID 游标、服务端筛选与前端分页', 'v0.2.2 · stable hardware-event ID cursor, server-side filtering and UI pagination'), tx('v0.2.1 · 只读生产接收库、真实总数、游标分页与新鲜度', 'v0.2.1 · read-only production ingestion store, real totals, cursor pagination and freshness'), tx('v0.2.0 · 接收记录、硬件事件与故障案例分层', 'v0.2.0 · ingestion, hardware event and fault case layers'), tx('v0.1.0 · open / recovered 事件生命周期', 'v0.1.0 · open/recovered event lifecycle')] },
    { id: 'issue', name: tx('数据统计', 'Data Statistics'), version: 'v0.3.0', status: tx('数据问题闭环基线', 'DATA ISSUE WORKFLOW'), desc: tx('统计当前资产范围内的数据质量、资产身份与节点可用性问题；硬件故障归入告警中心，底层处置记录继续用于训练数据。', 'Analyze data quality, inventory identity and node availability issues in the current asset scope. Hardware faults live in Alert Center while their resolution records remain available for training data.'), history: [tx('v0.3.0 · 更名为数据统计，硬件故障迁入告警中心且保留底层处置台账', 'v0.3.0 · renamed Data Statistics; hardware faults moved to Alert Center with resolution records retained'), tx('v0.2.3 · 恢复节点的 GPU/Target 状态同批更新并自动退出遗留统计', 'v0.2.3 · recovered GPU/target state updates in one run and automatically leaves remaining totals'), tx('v0.2.2 · 退休节点历史 Target 问题自动清除并退出活跃统计', 'v0.2.2 · historical target issues on retired nodes are automatically cleared and removed from active analytics'), tx('v0.2.1 · 双源数值差异退出列表、统计和训练数据', 'v0.2.1 · source-value differences left lists, analytics and training data')] },
  ...
  return <>
  <div className="grid">
    <Card className="span-12 product-intro"><div><span>{platformConfig.product_name}</span><h2>Infrastructure Hardware Reliability Workbench</h2><p>{tx('ATLAS 是面向 GPU 集群并可扩展至服务器、存储和网络基础设施的硬件可靠性工作台，提供资产对账、监控数据质量发现、硬件健康评分、故障检测、数据统计与处置、硬件故障预警与预测、性能衰减识别、告警中心以及维修验证闭环。', 'ATLAS is a hardware reliability workbench for GPU clusters, extensible to server, storage and network infrastructure. It provides asset reconciliation, monitoring data quality detection, hardware health scoring, fault detection, data analytics and resolution, hardware early warning and failure prediction, performance degradation analysis, an alert center and repair validation workflows.')}</p></div><Badge value="PLATFORM / v0.7.0" kind="info" /></Card>
