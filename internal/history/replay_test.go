...
func TestReplayOneRowComparesSharedStatistics(t *testing.T) {
	...
	columns := []string{"gpu_temp_mean_24h", "gpu_temp_slope_per_hour_24h", "gpu_temp_mean_1h", "gpu_temp_mean_30d"}
	...
	row := trainingMatrixRow{
		...
		Features:        map[string]float64{"gpu_temp_mean_24h": 11, "gpu_temp_slope_per_hour_24h": 2, "gpu_temp_mean_1h": 11, "gpu_temp_mean_30d": 11},
	}
	service := &Service{timeout: time.Second}
	result := service.replayOneRow(client, row, columns, results)
	if result.Status != "completed" || result.Compared != 4 || result.Mismatched != 0 || result.MissingReplay != 0 {
	...
	results["gpu_temp_mean_30d"] = &replayColumnResult{}
	result = service.replayOneRow(client, row, columns, results)
