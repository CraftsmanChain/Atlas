...
**Atlas** 是一个用于增强运维平台能力的 SRE 自动化与硬件可靠性平台。它不替代现有运维平台，而是作为 **运维能力增强层**，旨在解决告警信息不足、故障日志分散、硬件健康缺乏评估以及故障缺乏自动化处理能力等痛点。
...
### 1. Pull 模式 (运维平台调用 Atlas)
运维平台通过 API 主动调用 Atlas 获取数据：
```mermaid
graph LR
    Ops[运维平台] -->|API Call| Atlas[Atlas API]
    Atlas --> Analysis[告警分析 / 数据查询]
```
示例接口：
- `GET /api/alerts`
- `GET /api/node/health`
...
### 2. Push 模式 (Atlas 主动接收告警)
运维平台将告警 webhook 同步发送到 Atlas，Atlas 处理后可能反向推送结果或执行操作：
```mermaid
graph TD
...
### 告警增强
Atlas 可以对告警进行增强，提供翻译、原因分析和处理建议。
**示例**：
...
│
├── cmd                  # 各服务启动入口（main程序）
│   ├── gateway          # Atlas 事件入口服务
│   ├── api              # Atlas 对外 API 服务
│   └── agent            # 节点 Agent
│
├── internal             # Atlas 核心业务逻辑模块（不对外暴露）
...
│   ├── api              # Atlas API 定义（REST/gRPC 接口定义）
│   └── knowledge        # 故障知识库模块（常见故障模式、处理建议存储）
│
├── web                  # Atlas Web UI（告警展示、日志查看、健康评分看板）
└── deploy               # 部署配置 (Dockerfile, K8s YAML, Helm charts)
```

## Roadmap (路线图)

Atlas 的演进分为三个阶段，贯穿始终的核心能力是 **运维知识沉淀 (Ops Knowledge Base)**。
...
随着 **Ops Knowledge Base** 的不断丰富和历史数据的积累，Atlas 将从“辅助运维”向“自治运维”演进，接入 **AI Agent**。
...
Atlas 的长期核心价值在于 **运维经验结构化**：将脑子里的经验转化为系统里的知识库，再演进为自动化和 AI 能力。
...
Atlas 支持 Docker, Kubernetes, Helm 等多种部署方式，配置文件位于 `deploy/` 目录。

## 仓库地址

GitLab: [http://10.255.151.17:9091/sre/Atlas.git](http://10.255.151.17:9091/sre/Atlas.git)
