...
func main() {
	...
	if err != nil {
		...
		cfg = &config.Config{
			...
			NodeAccess: config.NodeAccessConfig{
				SkillID: "atlas-node-evidence", SkillVersion: "v0.3.1", SSHPort: 22,
	...
	nodeAccessHandler.SetPlanner(nodeaccess.NewPlanner(db, nodeAccessService))
	...
	mux.HandleFunc("/api/v1/node-access/plans", nodeAccessHandler.HandlePlans)
